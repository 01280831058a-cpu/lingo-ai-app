...
import {
  ...
  Volume2, VolumeX, ArrowDownAZ, Clock, Tag
} from 'lucide-react';
...
function MainApp({ user }: { user: User }) {
  ...
  const [sortOrder, setSortOrder] = useState<'newest'|'alpha'|'pos'>('newest');
  ...
  sortedDictWords.sort((a, b) => {
      if (sortOrder === 'alpha') return (a.original||'').localeCompare(b.original||'');
                   ...
                   {words.length > 0 && (
                      <>
                         ...
                         <div className="flex gap-2 mb-4 overflow-x-auto hide-scrollbar pb-1">
                            <button onClick={()=>setSortOrder('newest')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder==='newest'?'bg-stone-800 text-white':'bg-stone-200 text-stone-600 active:scale-95'}`}><Clock className="w-3 h-3"/> Новые</button>
                            <button onClick={()=>setSortOrder('alpha')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder==='alpha'?'bg-stone-800 text-white':'bg-stone-200 text-stone-600 active:scale-95'}`}><ArrowDownAZ className="w-3 h-3"/> А-Я</button>
                ...
                {activeTab.status === 'done' && (
                   activeTab.wordData?.translationOptions?.[0]?.includes('Ошибка ИИ') ? (
                      <div className="bg-rose-50 p-6 rounded-[2rem] border border-rose-100 text-center">
                         ...
                         <button onClick={() => handleAnalyzeWord({word: activeTab.original})} className="w-full py-4 bg-rose-500 text-white font-bold rounded-2xl active:scale-95 transition-transform flex justify-center items-center gap-2"><RefreshCw className="w-5 h-5"/> Попробовать еще раз</button>
                      </div>
                   ) : (
                     <div className="space-y-4">
...
function ModeConstructor({ words, onProgress, onFinish }: any) {
   ...
   useEffect(() => {
      ...
      ApiClient.playTTS(word.original);
   }, [word, currentNum]);
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  useEffect(() => {
      if (phase === 'playing' && word) ApiClient.playTTS(word.original);
...
function ModeFlashcards({ words, onProgress, onFinish }: any) {
  ...
  useEffect(() => { if (isFinished) onFinish(); }, [isFinished, onFinish]);
  useEffect(() => { if (word) ApiClient.playTTS(word.original); }, [word, currentNum]);
...
function ModeSentence({ words, onProgress, onFinish }: any) {
   ...
   useEffect(() => { if (isFinished) onFinish(); }, [isFinished, onFinish]);
   useEffect(() => { setInput(''); setStatus('idle'); setFb(''); if(word) ApiClient.playTTS(word.original); }, [word, currentNum]);
