...
class ApiClient {
  ...
  static async aiAnalyzeSpeech(audioBlob: Blob, topic: string, level?: string, mimeType?: string): Promise<{ transcript: string, feedback: string }> {
    try {
           ...
           reader.onloadend = async () => {
               ...
               try {
                   const res = await fetch(`${this.BASE_URL}/ai`, { 
                       ...
                       body: JSON.stringify({ action: 'analyze_speech', audio: base64Audio, topic, level, mimeType }) 
                   });
                   if (!res.ok) throw new Error('API Error');
  ...
  static async aiGenerateTopic(level?: string): Promise<string> {
    try {
       const res = await fetch(`${this.BASE_URL}/ai`, { 
           method: 'POST', 
           headers: { 'Content-Type': 'application/json' }, 
           body: JSON.stringify({ action: 'generate_topic', level }) 
       });
       if (!res.ok) throw new Error('API Error');
       const data = await res.json();
       return data.topic || "Describe your favorite hobby and why you enjoy it.";
    } catch(e) { 
       return "Talk about a recent book you read or movie you watched."; 
    }
  }
}
...
function ModeSpeaking({ onFinish, userProfile }: any) {
   ...
   const handleGenerateTopic = async () => {
      ...
      const newTopic = await ApiClient.aiGenerateTopic(userProfile?.level);
      setTopic(newTopic);
   ...
   const startRecording = async () => {
      ...
      try {
         ...
         mediaRecorder.current.onstop = async () => {
            ...
            setStatus('analyzing');
            const res = await ApiClient.aiAnalyzeSpeech(audioBlob, topic, userProfile?.level, mimeType);
            if (res.error) {
               setResult({ transcript: '', feedback: `Ошибка STT: ${res.error}` });
            } else {
               setResult(res);
            }
            setStatus('done');
            ...
            {status === 'idle' && (
               <div className="space-y-3">
                  ...
                  <button 
                     ...
                     className="text-xs font-bold text-rose-600 flex items-center justify-center gap-1 w-full active:scale-95 disabled:opacity-50"
                  >
                     {isGeneratingTopic ? <Loader2 className="w-3 h-3 animate-spin"/> : <Wand2 className="w-3 h-3"/>}
                     {isGeneratingTopic ? 'Генерируем...' : 'Сгенерировать случайную тему'}
                  </button>
