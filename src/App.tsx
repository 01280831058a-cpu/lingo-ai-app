...
class ApiClient {
  ...
  static async aiGenerateDistractors(word: string, correctTranslation: string): Promise<string[]> {
    try {
      const res = await fetch(`${this.BASE_URL}/ai`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'distractors', word, correctTranslation }) });
      if (!res.ok) throw new Error('API Error'); return await res.json();
    } catch(e) { return ['Случайное', 'Ошибочный', 'Другой']; }
  }
        ...
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
          {!activeTrainingMode && !viewingGroupId && activeTab !== 'settings' && activeTab !== 'train' && (
            <div className="sticky top-0 z-30 bg-[#F7F7F5]/80 backdrop-blur-xl pt-12 md:pt-8 pb-4 px-4 md:px-8 border-b border-stone-200/50">
              <h1 className="text-3xl font-bold tracking-tight text-stone-800">{activeTab === 'dict' ? 'Ваш словарь' : 'Группы слов'}</h1>
            </div>
          )}
          ...
          <AnimatePresence mode="wait">
            {!activeTrainingMode && !viewingGroupId && activeTab === 'dict' && (
              <motion.div key="dict" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-4 md:p-8 space-y-3 pb-32">
                ...
                {/* Скрываем кнопку +, если выбраны слова, чтобы она не перекрывала панель действий на мобильных */}
            ...
            {!activeTrainingMode && !viewingGroupId && activeTab === 'groups' && (
              <motion.div key="groups" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-4 md:p-8 space-y-3">
            ...
            {!activeTrainingMode && !viewingGroupId && activeTab === 'train' && (
              <motion.div key="train" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 md:p-8 pt-12 md:pt-8">
            ...
            {!activeTrainingMode && !viewingGroupId && activeTab === 'settings' && (
              <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 md:p-8 pt-12 md:pt-8 flex flex-col justify-between h-full">
                <div><h1 className="text-3xl font-bold tracking-tight mb-8 text-stone-800">Настройки</h1>
                  <div className="bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 mb-6">
                     <div className="font-bold text-lg text-stone-800 mb-1">{user?.displayName || 'Пользователь'}</div><div className="text-stone-500 text-sm mb-6">{user?.email}</div>
                     <div className="border-t border-stone-100 pt-6"><h3 className="font-bold text-stone-800 mb-4">Уровень языка</h3>
                        <div className="grid grid-cols-2 gap-2">{LEVELS.map((lvl) => <button key={lvl} onClick={() => setDoc(doc(db, 'users', user.uid, 'profile', 'data'), { ...userProfile, level: lvl }, { merge: true })} className={`py-3 text-xs font-bold rounded-xl transition-all ${userProfile?.level === lvl ? 'bg-teal-600 text-white shadow-sm' : 'bg-stone-50 text-stone-500 hover:bg-stone-100'}`}>{lvl}</button>)}</div>
                     </div>
                  </div>
                </div>
                <div className="space-y-3"><button onClick={() => signOut(auth)} className="w-full py-4 bg-stone-200/50 text-stone-700 font-bold rounded-2xl active:scale-95 transition-transform">Выйти</button><button onClick={handleDeleteAccount} className="w-full py-4 bg-orange-50 text-orange-500 font-bold rounded-2xl active:scale-95 transition-transform flex items-center justify-center gap-2"><AlertTriangle className="w-4 h-4"/> Удалить аккаунт</button></div>
              </motion.div>
            )}
          </AnimatePresence>
...
function AddWordModal({ userProfile, groups, onClose, onSave }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[60] flex flex-col justify-end">
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] mx-auto max-w-lg">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          ...
          {status === 'idle' && <button onClick={handleAnalyze} disabled={!original.trim()} className="w-full py-5 bg-teal-600 text-white font-bold rounded-[2rem] active:scale-95 transition-transform disabled:opacity-50">Анализировать ИИ</button>}
...
function BulkAddGroupModal({ groups, onClose, onSave, onOpenAddGroup }: any) {
   return (
      <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[60] flex flex-col justify-end max-w-md mx-auto">
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full rounded-t-[2rem] p-6 pb-12 shadow-2xl">
           ...
           <button onClick={()=>{onClose(); onOpenAddGroup();}} className="w-full mb-4 py-4 bg-teal-50 text-teal-700 font-bold rounded-2xl border border-teal-100 flex justify-center items-center gap-2"><Plus className="w-5 h-5"/> Создать группу</button>
...
function ModeConstructor({ words, onProgress, onFinish }: any) {
   ...
   useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
         ...
         if (availableLetter) { handlePick(availableLetter); }
      };
   ...
   const handlePick = (item: any) => {
      const newLetters = letters.filter(l => l.id !== item.id);
...
function ModeFlashcards({ words, onProgress, onFinish }: any) {
  ...
  const handleAnswer = (isCorrect: boolean) => {
    ...
    }, 250); // Ждем пока карточка перевернется обратно, потом меняем слово
  };
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  useEffect(() => {
    if (!word) return;
    const load = async () => { setOptions([]); setAnsIdx(null); const c = word.translation || ''; setCorrect(c); const dist = await ApiClient.aiGenerateDistractors(word.original, c); setOptions([...dist, c].sort(() => Math.random() - 0.5)); };
    load();
  }, [word]);
  ...
  return (
    <div className="w-full max-w-sm flex flex-col">
       ...
       {options.length === 0 ? <div className="flex justify-center p-8"><Loader2 className="w-8 h-8 animate-spin text-teal-600"/></div> : (
          <div className="space-y-3">
             {options.map((opt, i) => {
                let stateClass = "bg-white text-stone-800 border-stone-200";
                if (ansIdx !== null) { if (opt === correct) stateClass = "bg-teal-500 text-white border-transparent"; else if (i === ansIdx) stateClass = "bg-orange-400 text-white border-transparent"; else stateClass = "bg-white/50 text-stone-400 opacity-50"; }
                return <button key={i} onClick={() => { if(ansIdx===null) { setAnsIdx(i); onProgress(word.id, opt === correct); setTimeout(() => { if(currentIndex >= words.length-1) onFinish(); else setCurrentIndex(c=>c+1); }, 1500) } }} className={`w-full p-5 rounded-2xl border font-bold text-lg active:scale-[0.98] transition-all shadow-sm ${stateClass}`}>{opt}</button>;
             })}
          </div>
       )}
    </div>
