             ...
             <>
                ...
                {activeTab.status === 'done' && (
                  <div className="space-y-4">
                     ...
                     {groups && groups.length > 0 && <div className="px-2 pt-2"><h3 className="text-sm font-bold text-stone-400 mb-3">Добавить в группы:</h3><div className="flex flex-wrap gap-2">{groups.map((g: Group) => <button key={g.id} onClick={() => { const s = new Set(activeTab.groupIds || []); s.has(g.id) ? s.delete(g.id) : s.add(g.id); updateWordTab(activeTabId, { groupIds: s }); }} className={`px-4 py-2 rounded-xl text-sm font-bold border transition-colors flex items-center gap-2 ${(activeTab.groupIds || new Set()).has(g.id) ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white border-stone-200 text-stone-500'}`}>{(activeTab.groupIds || new Set()).has(g.id) && <Check className="w-4 h-4"/>} {g.name}</button>)}</div></div>}
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          ...
          {activeTab.status === 'done' && (
            <div className="space-y-4">
               ...
               {groups && groups.length > 0 && <div className="px-2 pt-2"><h3 className="text-sm font-bold text-stone-400 mb-3">Добавить в группы:</h3><div className="flex flex-wrap gap-2">{groups.map((g: Group) => <button key={g.id} onClick={() => { const s = new Set(activeTab.groupIds || []); s.has(g.id) ? s.delete(g.id) : s.add(g.id); updateTab(activeTabId, { groupIds: s }); }} className={`px-4 py-2 rounded-xl text-sm font-bold border transition-colors flex items-center gap-2 ${(activeTab.groupIds || new Set()).has(g.id) ? 'bg-teal-600 border-teal-600 text-white' : 'bg-white border-stone-200 text-stone-500'}`}>{(activeTab.groupIds || new Set()).has(g.id) && <Check className="w-4 h-4"/>} {g.name}</button>)}</div></div>}
               <button onClick={handleSaveTab} className="w-full py-5 mt-4 bg-stone-900 text-white font-bold rounded-[2rem] active:scale-95 transition-transform">Сохранить и закрыть вкладку</button>
            </div>
          )}
        </div>
...
function WordEditorModal({ word, words, groups, userProfile, user, onClose, onSave, onDelete, onReset, onWordClick }: any) {
  if (!word) return null; // Safe guard for unmounting state
