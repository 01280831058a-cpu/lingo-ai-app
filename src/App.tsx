...
function AddWordModal({ words, userProfile, groups, onClose, onSave, onWordClick, user }: any) {
  ...
  const relatedTabs = wordData.relatedWords || [];
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          ...
          {status === 'idle' && (
             <>
                ...
                <button onClick={handleAnalyze} disabled={!original.trim()} className="w-full py-5 bg-teal-600 text-white font-bold rounded-[2rem] active:scale-95 transition-transform disabled:opacity-50">Найти перевод</button>
             </>
          )}
          ...
          {status === 'done' && (
            <div className="space-y-4">
               ...
               <div className="flex overflow-x-auto gap-2 pb-2 mb-2 hide-scrollbar">
                   <button className="px-4 py-2 bg-teal-600 text-white rounded-xl text-sm font-bold shrink-0 shadow-sm">{wordData.original}</button>
                      ...
                      <div>
                         ...
                         <div className="flex flex-wrap justify-center gap-2 mb-2">
                            {wordData.translationOptions?.map((opt:string, i:number) => (
                               <button key={i} onClick={()=>setSelectedTranslation(opt)} className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${selectedTranslation === opt ? 'bg-teal-500 text-white border-teal-500 shadow-sm' : 'bg-white text-stone-600 border-stone-200'}`}>{opt}</button>
                            ))}
                         </div>
                      </div>
...
function WordEditorModal({ word, words, groups, userProfile, user, onClose, onSave, onDelete, onReset, onWordClick }: any) {
  ...
  // Когда меняется слово (переключение вкладки), обновляем стейты редактора
  useEffect(() => {
  ...
  const handleAddRelated = async (rw: any) => {
     ...
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: rw.word, translation: result.translationOptions?.[0] || result.translation || rw.translation, createdAt: Date.now(), masteryLevel: 0 });
     setLoadingRelated(null);
     onWordClick(id); // Сразу переключаемся на новую вкладку
  };
        ...
        <div className="pb-8">
           ...
           {(status === 'correct' || status === 'incorrect') && (
              <button onClick={() => { 
                 ...
                 className="w-full bg-stone-800 text-white font-bold py-5 rounded-[2rem] active:scale-95"
              >
