    ...
    <div className="min-h-screen bg-[#F7F7F5] font-sans text-stone-800 md:flex flex-row relative overflow-hidden">
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-y-auto ${!activeTrainingMode ? 'md:ml-64 pb-24 md:pb-0' : ''}`}>
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
          ...
          <AnimatePresence>
            {showAddWord && <AddWordModal user={user} userProfile={userProfile} groups={groups} onClose={() => setShowAddWord(false)} onSaveWord={(w:any) => { const id=doc(collection(db,'users',user.uid,'words')).id; setDoc(doc(db,'users',user.uid,'words',id),{...w,id,createdAt:Date.now(),masteryLevel:0}); }} words={words} />}
            {viewingWordId && <WordEditorModal word={words.find(w=>w.id===viewingWordId)!} groups={groups} userProfile={userProfile} onClose={() => setViewingWordId(null)} onReset={() => setAppConfirm({title:"Сброс прогресса", message:"Сбросить прогресс этого слова?", onConfirm: ()=>resetProgress([viewingWordId])})} onSave={(w:any) => { updateDoc(doc(db,'users',user.uid,'words',w.id),w); }} onDelete={() => { setAppConfirm({title:"Удаление", message:"Удалить это слово из словаря?", onConfirm:()=>{deleteWords([viewingWordId!]); setViewingWordId(null);}}) }} />}
...
function AddWordModal({ userProfile, groups, words, onClose, onSaveWord, user }: any) {
