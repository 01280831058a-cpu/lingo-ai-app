                            ...
                            return (
                               <div key={i} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-stone-200 shadow-sm gap-3">
                                  ...
                                  <div className="flex-1 cursor-pointer clickable-card" onClick={() => toggleSelect(w.word)}>
                                     <span className="font-bold text-stone-800 text-lg">{cleanWWord}</span>
                                     <div className="text-sm text-stone-500">{(w.translation || '').toLowerCase()}</div>
                                  </div>
                                  {existingInDict ? (
                         ...
                         <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                            <div className="flex flex-col items-center justify-center mb-4 gap-2">
                               <div className="flex items-center justify-center gap-2">
                                  {activeTab.wordData?.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-3 py-1 rounded-lg text-xs uppercase tracking-wide">{activeTab.wordData.partOfSpeech}</span>}
                                  <button onClick={() => ApiClient.playTTS(activeTab.wordData?.original || activeTab.original)} className="w-8 h-8 flex items-center justify-center bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200 active:scale-90 transition-all">
                                     <Volume2 className="w-4 h-4"/>
                                  </button>
                               </div>
                               <h3 className="text-4xl font-black text-stone-800 text-center">{(activeTab.wordData?.original || activeTab.original || '').toLowerCase()}</h3>
                            </div>
                ...
                <div className="space-y-4">
                   <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                      <div className="flex flex-col items-center justify-center mb-4 gap-2">
                         <div className="flex items-center justify-center gap-2">
                            {activeTab.wordData?.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-3 py-1 rounded-lg text-xs uppercase tracking-wide">{activeTab.wordData.partOfSpeech}</span>}
                            <button onClick={() => ApiClient.playTTS(activeTab.wordData?.original || activeTab.original)} className="w-8 h-8 flex items-center justify-center bg-teal-100 text-teal-700 rounded-full hover:bg-teal-200 active:scale-90 transition-all">
                               <Volume2 className="w-4 h-4"/>
                            </button>
                         </div>
                         <h3 className="text-4xl font-black text-stone-800 text-center">{(activeTab.wordData?.original || activeTab.original || '').toLowerCase()}</h3>
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           ...
           <div className="flex-1 overflow-y-auto space-y-4 pb-6 hide-scrollbar">
              <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                 <div className="flex flex-col items-center justify-center mb-4 gap-2">
                    <div className="flex items-center justify-center gap-2">
                       {word.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-3 py-1 rounded-lg text-xs uppercase tracking-wide">{word.partOfSpeech}</span>}
                       <button onClick={() => ApiClient.playTTS(word.original)} className="w-8 h-8 flex items-center justify-center bg-teal-100 text-teal-700 rounded-full hover:bg-teal-200 active:scale-90 transition-all">
                          <Volume2 className="w-4 h-4"/>
                       </button>
                    </div>
                    <h3 className="text-4xl font-black text-stone-800 text-center">{(word.original || '').toLowerCase()}</h3>
                 </div>
