...
export const getEffectiveMastery = (word: Partial<Word>) => {
    if (!word) return 0;
    const rawMastery = Number(word.masteryLevel) || 0;
    const lastPrac = Number(word.lastPracticed) || 0;
    if (!lastPrac) return rawMastery;
    
    const diffMs = Date.now() - lastPrac;
    const weeksPassed = Math.floor(diffMs / (1000 * 60 * 60 * 24 * 7));
    
    if (Number.isNaN(weeksPassed) || weeksPassed < 1) return rawMastery;
