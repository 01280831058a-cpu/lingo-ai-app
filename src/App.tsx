...
import {
  ...
  LogOut, Wand2, GraduationCap, Download, Mail, Calendar, RotateCcw, Link2
} from 'lucide-react';
...
// --- QUEUE ENGINE (REPEATS WRONG WORDS) ---
function useTrainingQueue(initialWords: any[]) {
   const [queue, setQueue] = useState<any[]>(initialWords);
   const [idx, setIdx] = useState(0);

   // Sync if initial array actually changes completely
   useEffect(() => { setQueue(initialWords); setIdx(0); }, [initialWords]);
   
   const handleNext = useCallback((word: any, isCorrect: boolean) => {
      setQueue(prev => {
         const newQueue = [...prev];
         if (!isCorrect) newQueue.push(word); // Закидываем в конец очереди при ошибке
         return newQueue;
      });
      setIdx(c => c + 1);
   }, []);
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         if (isLogin) {
            ...
            if (!res.user.emailVerified) { alert('Подтвердите почту. Мы отправили вам письмо.'); await sendEmailVerification(res.user); }
         } else {
            const res = await createUserWithEmailAndPassword(auth, email, password);
            await updateProfile(res.user, { displayName: name });
            await sendEmailVerification(res.user);
            alert('Регистрация успешна! Проверьте почту для подтверждения (и папку Спам).');
         }
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#F7F7F5]">
        <div className="bg-white/60 p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl border border-white/60 w-full max-w-sm flex flex-col items-center">
           ...
           <div className="w-full flex justify-between items-center mt-4 text-sm font-bold">
              ...
              {isLogin && <button onClick={async () => { if(!email) return alert('Введите email'); try { await sendPasswordResetEmail(auth, email); alert('Письмо отправлено!'); } catch(e:any) { alert(e.message); } }} className="text-teal-600">Забыли пароль?</button>}
           </div>
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-y-auto ${!activeTrainingMode ? 'md:ml-64 pb-24 md:pb-0' : ''}`}>
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
               ...
               {!activeTrainingMode && !viewingGroupId && activeTab === 'settings' && (
                 <motion.div key="tab-settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8">
                     ...
                     <div className="space-y-3">
                       ...
                       <button onClick={() => setShowDeleteConfirm(true)} className="w-full py-4 text-stone-400 font-bold text-sm active:scale-95 transition-transform">Навсегда удалить аккаунт</button>
                     </div>
                 </motion.div>
               )}
            </AnimatePresence>
          ...
          <AnimatePresence>
            ...
            {viewingWordId && <WordEditorModal word={words.find(w=>w.id===viewingWordId)!} groups={groups} userProfile={userProfile} onClose={() => setViewingWordId(null)} onReset={() => resetProgress([viewingWordId])} onSave={(w:any) => { updateDoc(doc(db,'users',user.uid,'words',w.id),w); setViewingWordId(null); }} onDelete={() => { deleteWords([viewingWordId!]); setViewingWordId(null); }} />}
            ...
            {showDeleteConfirm && <DeleteAccountModal onClose={() => setShowDeleteConfirm(false)} onDelete={async (pwd: string) => { try { const cred = EmailAuthProvider.credential(user.email!, pwd); await reauthenticateWithCredential(user, cred); await resetEntireDictionary(); await deleteUser(user); } catch(e:any) { alert("Ошибка: Неверный пароль."); } }} />}
          </AnimatePresence>
        </div>
      </main>
...
function AddWordModal({ userProfile, groups, onClose, onSave, user }: any) {
  ...
  const handleAnalyze = async () => {
    ...
    setWordData(result);
    setSelectedTranslation(result.translationOptions?.[0] || result.translation || '');
    setStatus('done');
  };

  const addRelatedWord = async (relatedWord: string) => {
     setLoadingRelated(relatedWord);
     const result = await ApiClient.aiGenerateWord(relatedWord, userProfile?.level);
     const id = doc(collection(db, 'users', user.uid, 'words')).id;
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: relatedWord, translation: result.translationOptions?.[0] || result.translation, createdAt: Date.now(), masteryLevel: 0 });
    ...
    <div className="fixed inset-0 z-[60] flex flex-col justify-end items-center">
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          ...
          {status === 'idle' && <button onClick={handleAnalyze} disabled={!original.trim()} className="w-full py-5 bg-teal-600 text-white font-bold rounded-[2rem] active:scale-95 transition-transform disabled:opacity-50">Найти перевод</button>}
          {status === 'analyzing' && <div className="py-12 flex flex-col items-center justify-center text-stone-500"><Loader2 className="w-8 h-8 animate-spin text-teal-600 mb-2" /> Изучаем контекст (или ищем в кэше)...</div>}
          {status === 'done' && (
            <div className="space-y-4">
               <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                  ...
                  <div className="space-y-5">
                      <div>
                         ...
                         <div className="flex flex-wrap justify-center gap-2 mb-2">
                            {wordData.translationOptions?.map((opt:string, i:number) => (
                               <button key={i} onClick={()=>setSelectedTranslation(opt)} className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${selectedTranslation === opt ? 'bg-teal-500 text-white border-teal-500' : 'bg-white text-stone-600 border-stone-200'}`}>{opt}</button>
                            ))}
                         </div>
                      </div>
                  ...
                  {wordData.relatedWords && wordData.relatedWords.length > 0 && (
                     <div className="mt-6 pt-4 border-t border-stone-200/50">
                        <div className="text-[10px] font-bold text-stone-400 uppercase mb-3 text-center">Однокоренные слова (Части речи)</div>
                        <div className="space-y-2">
                           {wordData.relatedWords.map((rw:any, i:number) => (
                              <div key={i} className="flex items-center justify-between bg-white p-3 rounded-xl border border-stone-200 shadow-sm">
                                 <div><span className="font-bold text-stone-800">{rw.word}</span> <span className="text-xs text-stone-400 bg-stone-100 px-1.5 py-0.5 rounded">{rw.partOfSpeech}</span><div className="text-xs text-stone-500">{rw.translation}</div></div>
                                 <button onClick={()=>addRelatedWord(rw.word)} disabled={loadingRelated === rw.word} className="w-8 h-8 flex items-center justify-center bg-teal-50 text-teal-600 rounded-full active:scale-90 disabled:opacity-50">{loadingRelated === rw.word ? <Loader2 className="w-4 h-4 animate-spin"/> : <Plus className="w-5 h-5"/>}</button>
                              </div>
                           ))}
                        </div>
                     </div>
               ...
               <button onClick={() => onSave({ original: wordData.original || original, ...wordData, translation: selectedTranslation, groupIds: Array.from(selectedGroups) })} className="w-full py-5 mt-4 bg-stone-900 text-white font-bold rounded-[2rem] active:scale-95 transition-transform">Добавить в словарь</button>
            </div>
          )}
        </div>
      </motion.div>
...
function WordEditorModal({ word, groups, userProfile, onClose, onSave, onDelete, onReset }: any) {
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           <div className="flex justify-between items-center mb-6 shrink-0">
              ...
              <div className="flex items-center gap-2">
                 <button onClick={()=>{ onReset(); onClose(); }} className="p-2 bg-orange-50 text-orange-500 rounded-full"><RotateCcw className="w-5 h-5" /></button>
...
function useTrainingQueue(initialWords: Word[]) {
   ...
   }, [initialWords]); // Только первичная инициализация для викторины
...
function ModeConstructor({ words, onProgress, onFinish }: any) {
   ...
   }, [word, currentNum]); // Добавлен currentNum, чтобы эффект срабатывал на повторе слова
...
function OnboardingModal({ user, onSave }: any) {
  ...
  const slides = [
    { 
       ...
       text: "Больше никаких гугл-переводчиков. Добавьте слово, и наша нейросеть (заточенная под лингвистику) выдаст идеальный перевод, британскую и американскую транскрипции, а также развернутое объяснение в стиле Cambridge Dictionary." 
    },
    { 
