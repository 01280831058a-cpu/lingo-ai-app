...
import {
  ...
  LogOut, Wand2, GraduationCap, Download, Mail, Lock, User as UserIcon, Calendar, RotateCcw
} from 'lucide-react';
...
class ApiClient {
  ...
  static async aiGenerateWord(word: string, level?: string): Promise<Partial<Word>> {
    ...
    // Если нет в кэше - генерируем и сохраняем в кэш
    try {
  ...
  static async aiCheckSentence(word: string, sentence: string): Promise<{ isCorrect: boolean, feedback: string }> {
    try {
      const res = await fetch(`${this.BASE_URL}/ai`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'check', word, sentence, level: localStorage.getItem('userLevel') || 'Intermediate' }) });
      if (!res.ok) throw new Error('API Error'); return await res.json();
...
function AuthScreen() {
   ...
   const handleGoogle = async () => { try { await signInWithPopup(auth, new GoogleAuthProvider()); } catch(e:any){ alert(e.message); } };
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         if (isLogin) {
            ...
            if (!res.user.emailVerified) { alert('Пожалуйста, подтвердите вашу почту. Мы отправили вам письмо.'); await sendEmailVerification(res.user); }
         } else {
            const res = await createUserWithEmailAndPassword(auth, email, password);
            await updateProfile(res.user, { displayName: name });
            await sendEmailVerification(res.user);
            alert('Регистрация успешна! Проверьте вашу почту для подтверждения (и папку Спам).');
         }
   ...
   const resetPassword = async () => {
      ...
      try { await sendPasswordResetEmail(auth, email); alert('Письмо отправлено!'); } catch(e:any) { alert(e.message); }
   };
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#F7F7F5]">
        <div className="bg-white/60 p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl border border-white/60 w-full max-w-sm flex flex-col items-center">
           ...
           <form onSubmit={handleSubmit} className="w-full space-y-4">
              {!isLogin && <input required value={name} onChange={e=>setName(e.target.value)} type="text" placeholder="Ваше имя" className="w-full bg-white px-4 py-4 rounded-2xl border border-stone-200 outline-none focus:border-teal-500 transition-colors" />}
              ...
              <input required value={password} onChange={e=>setPassword(e.target.value)} type="password" placeholder="Пароль" className="w-full bg-white px-4 py-4 rounded-2xl border border-stone-200 outline-none focus:border-teal-500 transition-colors" />
              ...
              <button type="submit" disabled={loading} className="w-full py-4 bg-teal-600 text-white font-bold rounded-2xl active:scale-95 transition-transform disabled:opacity-50">
                 {loading ? <Loader2 className="animate-spin mx-auto w-5 h-5"/> : isLogin ? 'Войти' : 'Зарегистрироваться'}
              </button>
           </form>

           <div className="w-full flex justify-between items-center mt-4 text-sm font-bold">
              <button onClick={()=>setIsLogin(!isLogin)} className="text-stone-500">{isLogin ? 'Создать аккаунт' : 'Уже есть аккаунт?'}</button>
              {isLogin && <button onClick={resetPassword} className="text-teal-600">Забыли пароль?</button>}
           </div>
...
export default function AppWrapper() {
  ...
  // Требуем верификацию email, если вход не через Google
...
function MainApp({ user }: { user: User }) {
  ...
  const resetProgress = (ids: string[]) => {
     ids.forEach(id => updateDoc(doc(db, 'users', user.uid, 'words', id), { correctAnswers: 0, incorrectAnswers: 0, masteryLevel: 0 }));
     setSelectedWordIds(new Set());
  };
  const resetAllProgress = () => {
     if(window.confirm('Сбросить весь прогресс изучения? Слова останутся.')) {
        words.forEach(w => updateDoc(doc(db, 'users', user.uid, 'words', w.id), { correctAnswers: 0, incorrectAnswers: 0, masteryLevel: 0 }));
     }
  };
  const resetEntireDictionary = async () => {
     if(window.confirm('ВНИМАНИЕ! Это удалит ВСЕ слова и группы навсегда! Вы уверены?')) {
  ...
  const handleDeleteAccount = async () => {
    const password = window.prompt("Для удаления аккаунта введите ваш текущий пароль:");
    if (password) {
       try {
         const cred = EmailAuthProvider.credential(user.email!, password);
         await reauthenticateWithCredential(user, cred);
         await resetEntireDictionary();
  ...
  if (userProfile && !userProfile.onboarded) return <OnboardingModal user={user} onSave={(level: string) => setDoc(doc(db, 'users', user.uid, 'profile', 'data'), { level, onboarded: true, dailyGoal: 15, activity: {}, streak: 0 }, { merge: true })} />;
    ...
    <div className="min-h-screen bg-[#F7F7F5] font-sans text-stone-800 md:flex flex-row relative overflow-hidden">
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-y-auto ${!activeTrainingMode ? 'md:ml-64 pb-24 md:pb-0' : ''}`}>
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
          ...
          <div className="flex-1 w-full relative">
            <AnimatePresence mode="wait">
               ...
               {!activeTrainingMode && !viewingGroupId && activeTab === 'dict' && (
                 <motion.div key="tab-dict" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8 space-y-3 pb-32">
                   ...
                   {words.length === 0 ? <div className="text-center text-stone-400 py-12">Словарь пуст.</div> : words.map(word => (
                     <div key={word.id} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 active:scale-[0.98] transition-transform">
                        ...
                        <div className="flex-1 cursor-pointer" onClick={() => setViewingWordId(word.id)}>
                          <div className="flex items-center gap-2"><h3 className="text-lg font-bold text-stone-800">{word.original}</h3> {word.partOfSpeech && <span className="text-[10px] font-bold text-stone-400 uppercase bg-stone-100 px-2 py-0.5 rounded-md">{word.partOfSpeech}</span>}</div>
                   ...
                   <BulkActions selectedWordIds={selectedWordIds} onTrain={() => setActiveTab('train')} onDelete={deleteWords} onReset={resetProgress} onAddToGroup={() => setShowBulkAddGroup(true)} />
                 </motion.div>
               )}

               {/* 2. Вкладка Группы */}
               {!activeTrainingMode && !viewingGroupId && activeTab === 'groups' && (
                 <motion.div key="tab-groups" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8 space-y-3">
               ...
               {!activeTrainingMode && !viewingGroupId && activeTab === 'train' && (
                 <motion.div key="tab-train" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8">
               ...
               {!activeTrainingMode && !viewingGroupId && activeTab === 'progress' && (
                 <motion.div key="tab-progress" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8">
                     ...
                     <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-stone-100">
                        <div className="grid grid-cols-7 gap-2">
                           {['Пн','Вт','Ср','Чт','Пт','Сб','Вс'].map(d=><div key={d} className="text-center text-xs font-bold text-stone-400">{d}</div>)}
                           {Array.from({length: 30}).map((_, i) => {
                               const d = new Date(); d.setDate(d.getDate() - (29 - i));
                               const dateStr = d.toISOString().split('T')[0];
                               const mins = userProfile?.activity?.[dateStr] || 0;
                               const isGoalReached = mins >= (userProfile?.dailyGoal || 15);
                               return <div key={i} className={`aspect-square rounded-xl flex items-center justify-center text-xs font-bold ${isGoalReached ? 'bg-orange-400 text-white' : mins > 0 ? 'bg-orange-200 text-orange-800' : 'bg-stone-50 text-stone-400'}`}>{d.getDate()}</div>
                           })}
                        </div>
                     </div>
                 </motion.div>
               ...
               {!activeTrainingMode && !viewingGroupId && activeTab === 'settings' && (
                 <motion.div key="tab-settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8">
                     ...
                     <div className="space-y-3">
                       ...
                       <button onClick={() => signOut(auth)} className="w-full py-4 bg-stone-900 text-white font-bold rounded-2xl active:scale-95 transition-transform flex justify-center items-center gap-2 mt-8"><LogOut className="w-5 h-5"/> Выйти</button>
          ...
          <AnimatePresence>
            {showAddWord && <AddWordModal user={user} userProfile={userProfile} groups={groups} onClose={() => setShowAddWord(false)} onSave={(w:any) => { const id=doc(collection(db,'users',user.uid,'words')).id; setDoc(doc(db,'users',user.uid,'words',id),{...w,id,createdAt:Date.now(),masteryLevel:0}); setShowAddWord(false); }} />}
   ...
   return (
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative z-10">
           ...
           <div className="bg-stone-50 p-4 rounded-2xl mb-6 border border-stone-200"><div className="flex justify-between text-xs font-bold text-stone-400 mb-2 uppercase"><span>Количество</span><span>Всего: {words.length}</span></div><input type="number" min="1" max={words.length} value={count} onChange={e => { let val = parseInt(e.target.value) || 1; if(val > words.length) val = words.length; setCount(val); }} className="w-full text-2xl font-black bg-transparent outline-none text-stone-800" /></div>
           <div className="flex gap-3"><button onClick={onClose} className="flex-1 py-4 bg-stone-100 text-stone-700 font-bold rounded-2xl active:bg-stone-200">Отмена</button><button onClick={() => { const sorted = [...words].sort((a, b) => (a.masteryLevel || 0) - (b.masteryLevel || 0)); onSelect(sorted.slice(0, count).map(w => w.id)); }} className="flex-1 py-4 bg-indigo-600 text-white font-bold rounded-2xl active:bg-indigo-700 flex justify-center items-center gap-2"><Brain className="w-4 h-4"/> Выбрать</button></div>
        </motion.div>
      </div>
   );
}

function AddWordModal({ userProfile, groups, onClose, onSave, user }: any) {
  ...
  const addAlternative = async (alt: string) => {
     setStatus('analyzing');
     const result = await ApiClient.aiGenerateWord(alt, userProfile?.level);
     const id = doc(collection(db, 'users', user.uid, 'words')).id;
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: alt, createdAt: Date.now(), masteryLevel: 0 });
     setStatus('done');
     alert(`Слово "${alt}" добавлено в словарь отдельной карточкой!`);
  };
    ...
    <div className="fixed inset-0 z-[60] flex flex-col justify-end items-center">
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          ...
          {status === 'analyzing' && <div className="py-12 flex flex-col items-center justify-center text-stone-500"><Loader2 className="w-8 h-8 animate-spin text-teal-600 mb-2" /> Ищем в глобальной базе...</div>}
          {status === 'done' && (
            <div className="space-y-4">
               <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                  <div className="flex items-center justify-center gap-2 mb-6">
                     <h3 className="text-3xl font-black text-stone-800">{wordData.original || original}</h3>
                     {wordData.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-2 py-1 rounded-lg text-xs uppercase">{wordData.partOfSpeech}</span>}
                  </div>
                  <div className="space-y-5">
                      <div><div className="text-[10px] font-bold text-stone-400 uppercase text-center mb-1">Краткий перевод</div><div className="font-bold text-stone-800 text-2xl text-center">{wordData.translation}</div></div>
                  ...
                  {wordData.alternativeTranslations && wordData.alternativeTranslations.length > 0 && (
                     <div className="mt-6 pt-4 border-t border-stone-200/50 text-center">
                        <div className="text-[10px] font-bold text-stone-400 uppercase mb-3">Другие значения (нажмите чтобы добавить)</div>
                        <div className="flex flex-wrap justify-center gap-2">
                           {wordData.alternativeTranslations.map((alt:string, i:number) => <button key={i} onClick={()=>addAlternative(alt)} className="px-3 py-1 bg-white border border-stone-200 rounded-xl text-sm font-bold text-stone-600 active:bg-stone-100">{alt}</button>)}
                        </div>
                     </div>
                  )}
               </div>
...
function WordEditorModal({ word, groups, userProfile, onClose, onSave, onDelete }: any) {
  ...
  return (
     <div className="fixed inset-0 z-[100] flex flex-col justify-end items-center">
        <div className="absolute inset-0 bg-stone-900/40 backdrop-blur-sm" onClick={onClose} />
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           <div className="flex justify-between items-center mb-6 shrink-0"><h2 className="text-xl font-bold text-stone-800 flex items-center gap-2"><Edit3 className="w-5 h-5 text-teal-600"/> Редактор</h2><div className="flex items-center gap-2"><button onClick={onDelete} className="p-2 bg-rose-50 text-rose-500 rounded-full mr-2"><Trash2 className="w-5 h-5" /></button><button onClick={onClose} className="p-2 bg-stone-100 rounded-full"><X className="w-5 h-5 text-stone-500" /></button></div></div>
           <div className="flex-1 overflow-y-auto space-y-4 pb-6 hide-scrollbar">
              ...
              <div className="space-y-1"><div className="flex justify-between items-center"><label className="text-[10px] font-bold text-stone-400 uppercase">Пример ИИ</label><button onClick={handleRegenerateExample} disabled={isRegenerating} className="text-[10px] font-bold text-teal-600 flex items-center gap-1 active:scale-95"><RefreshCw className={`w-3 h-3 ${isRegenerating?'animate-spin':''}`}/> Сменить</button></div><textarea value={exampleText} onChange={e => setExampleText(e.target.value)} className="w-full bg-teal-50 px-4 py-3 rounded-2xl border border-teal-100 outline-none focus:border-teal-500 text-sm min-h-[60px]" /></div>
   ...
   return (
      <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }} className="absolute inset-0 z-20 bg-[#F7F7F5] flex flex-col pb-24 top-0 pt-12 md:pt-8">
         <div className="flex items-center px-4 md:px-8 pb-4 border-b border-stone-200/50 sticky top-0 z-10 bg-[#F7F7F5]/80 backdrop-blur-xl">
            ...
            <button onClick={()=>{ if(window.confirm('Удалить группу? Слова останутся в общем словаре.')) onDeleteGroup(); }} className="p-2 bg-rose-50 text-rose-500 rounded-full"><Trash2 className="w-5 h-5"/></button>
         </div>
...
function SessionStats({ stats, onClose }: any) {
   ...
   return <div className="w-full text-center flex flex-col items-center"><div className="bg-white rounded-[2rem] p-10 w-full shadow-[0_10px_40px_rgb(0,0,0,0.05)] border border-stone-100 mb-8"><div className="text-6xl font-black text-teal-600 mb-2">{accuracy}%</div><div className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs">Точность</div><div className="flex justify-around text-lg font-bold"><div className="flex flex-col"><span className="text-teal-500 text-2xl">{stats.correct}</span><span className="text-stone-400 text-xs uppercase">Верно</span></div><div className="flex flex-col"><span className="text-orange-400 text-2xl">{stats.total - stats.correct}</span><span className="text-stone-400 text-xs uppercase">Ошибок</span></div></div></div><button onClick={onClose} className="w-full bg-teal-600 text-white font-bold py-5 rounded-[2rem] shadow-[0_8px_20px_rgb(13,148,136,0.3)]">Завершить</button></div>;
}

// Новая логика очереди тренировок
function useTrainingQueue(initialWords: Word[]) {
   const [queue, setQueue] = useState<Word[]>(initialWords);
   const [idx, setIdx] = useState(0);
   
   const handleNext = (word: Word, isCorrect: boolean) => {
      if (!isCorrect) setQueue(prev => [...prev, word]); // Если ошибка - слово идет в конец очереди
      setIdx(c => c + 1);
   };
...
function ModeConstructor({ words, onProgress, onFinish }: any) {
   ...
   useEffect(() => { if (isFinished) onFinish(); }, [isFinished]);
   useEffect(() => {
      if(!word) return;
   ...
   const processInput = (char: string, availableLetter?: {id:number, char:string}) => {
      ...
      if (char.toLowerCase() !== correctNextChar) {
         ...
         if (newErrors >= 3) {
            // Принудительно показываем правильное слово и переходим
            setAnswer(word.original.split('').map((c:string,i:number)=>({id:i,char:c})));
            setLetters([]);
   ...
   const handleKeyDown = useCallback((e: KeyboardEvent) => {
      if(errorsCount >= 3 || answer.length === word?.original.length) return;
   ...
   }, [letters, answer, word, errorsCount]);
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  useEffect(() => {
    const fetchBatch = async () => {
      ...
      const formatted = words.map((w: any) => {
         ...
         return { ...w, options: [...dists, w.translation].sort(() => Math.random() - 0.5) };
      });
  ...
  useEffect(() => { if (isFinished && phase === 'playing') onFinish(); }, [isFinished, phase]);
  ...
  return (
    <div className="w-full max-w-sm flex flex-col">
       ...
       <div className="bg-white rounded-[2rem] p-8 text-center mb-8 border border-stone-100 shadow-sm"><h2 className="text-3xl font-black text-stone-800">{word.original}</h2></div>
       <div className="space-y-3">
          {word.options.map((opt: string, i: number) => {
             let stateClass = "bg-white text-stone-800 border-stone-200";
             if (ansIdx !== null) { if (opt === word.translation) stateClass = "bg-teal-500 text-white border-transparent"; else if (i === ansIdx) stateClass = "bg-orange-400 text-white border-transparent"; else stateClass = "bg-white/50 text-stone-400 opacity-50"; }
             return <button key={i} onClick={() => { if(ansIdx===null) { setAnsIdx(i); const isCorrect = opt === word.translation; onProgress(word.id, isCorrect); setTimeout(() => { handleNext(word, isCorrect); setAnsIdx(null); }, 1500) } }} className={`w-full p-5 rounded-2xl border font-bold text-lg active:scale-[0.98] transition-all shadow-sm ${stateClass}`}>{opt}</button>;
          })}
       </div>
    </div>
  );
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full h-96 relative cursor-pointer perspective-1000" onClick={() => setIsFlipped(!isFlipped)}>
         <motion.div animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ type: "spring", stiffness: 100, damping: 20 }} className="w-full h-full relative [transform-style:preserve-3d]">
            <div className="absolute inset-0 [backface-visibility:hidden] bg-white rounded-[2rem] shadow-[0_10px_40px_rgb(0,0,0,0.05)] border border-stone-100 flex flex-col items-center justify-center p-8 text-center"><h2 className="text-4xl font-black text-stone-800">{word.original}</h2><div className="text-stone-400 mt-4 font-medium">[{word.transcriptionUK}]</div></div>
            <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] bg-teal-600 rounded-[2rem] shadow-[0_10px_40px_rgb(13,148,136,0.2)] flex flex-col items-center justify-center p-8 text-center text-white"><h2 className="text-3xl font-bold mb-2">{word.translation}</h2><p className="text-teal-100/80 text-sm mb-4">{word.cambridgeTranslation}</p>{word.examples?.[0] && <p className="italic text-teal-50 bg-black/10 p-3 rounded-xl text-sm">"{word.examples[0].text}"</p>}</div>
         </motion.div>
      </div>
   ...
   return (
     <div className="w-full flex flex-col h-[80vh]">
        <div className="flex-1">
          ...
          {status !== 'idle' && status !== 'checking' && <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`mt-4 p-5 rounded-2xl flex gap-3 ${status === 'correct' ? 'bg-teal-50 text-teal-700' : 'bg-orange-50 text-orange-600'}`}>{status === 'correct' ? <CheckCircle2 className="w-6 h-6 shrink-0"/> : <XCircle className="w-6 h-6 shrink-0"/><p className="font-medium text-sm leading-relaxed">{fb}</p></motion.div>}
        </div>
        <div className="pb-8">
...
function ModeBrainstorm({ words, onProgress, onFinish }: any) {
   ...
   if (phase === 1) return (
         <div className="w-full"><h2 className="text-2xl font-black text-stone-800 mb-2">Брейншторм</h2><p className="text-stone-500 mb-6">Выберите слова для усиленного цикла.</p><div className="space-y-2 mb-8 max-h-[50vh] overflow-y-auto">{words.map((w:any) => <div key={w.id} onClick={() => { const s=new Set(selected); s.has(w.id)?s.delete(w.id):s.add(w.id); setSelected(s); }} className={`p-4 rounded-2xl flex justify-between font-bold cursor-pointer transition-colors ${selected.has(w.id) ? 'bg-purple-100 text-purple-700' : 'bg-white text-stone-700'}`}>{w.original} {selected.has(w.id) && <Check className="w-5 h-5"/>}</div>)}</div><button onClick={() => { if(selected.size>0) setPhase(2); else alert('Выберите слова'); }} className="w-full bg-purple-600 text-white font-bold py-5 rounded-[2rem]">Начать цикл ({selected.size})</button></div>
   );
   if (phase === 2) return <div className="w-full text-center"><h3 className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs">Этап 1: Викторина</h3><ModeQuiz words={activeWords} onProgress={onProgress} onFinish={() => setPhase(3)} /></div>;
...
function OnboardingModal({ user, onSave }: any) {
  ...
  const slides = [
    { icon: <Wand2 className="w-8 h-8 text-teal-600" />, title: "Магия ИИ", text: "Словарь автоматически найдет перевод, добавит кембриджское объяснение и озвучит слово." },
    { icon: <Edit3 className="w-8 h-8 text-sky-600" />, title: "Полный контроль", text: "Группируйте слова, сбрасывайте прогресс, редактируйте переводы и учитесь так, как удобно вам." },
    { icon: <Brain className="w-8 h-8 text-purple-600" />, title: "Умные тренировки", text: "Конструктор следит за ошибками, а викторина заставляет повторять слабые слова, пока вы их не запомните." },
    { icon: <Calendar className="w-8 h-8 text-orange-400" />, title: "Ваш прогресс", text: "Выбирайте цель на день, следите за статистикой в календаре и бейте рекорды ударного режима (стрик)!" }
  ];
      ...
      <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} transition={{type:'spring', stiffness: 200, damping: 25}} className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-[0_20px_60px_rgb(0,0,0,0.1)] relative z-10">
        {step < 4 ? (
          <div className="p-8 text-center">
            <div className="w-16 h-16 mx-auto bg-stone-50 rounded-2xl flex items-center justify-center mb-6">{slides[step].icon}</div>
            <h2 className="text-2xl font-black mb-4 text-stone-800">{slides[step].title}</h2><p className="text-stone-500 leading-relaxed">{slides[step].text}</p>
            <div className="flex justify-center gap-2 mt-8">{[0,1,2,3].map(i => <div key={i} className={`w-2 h-2 rounded-full transition-colors ${step === i ? 'bg-teal-500' : 'bg-stone-200'}`} />)}</div>
          </div>
        ) : (
          <div className="p-8">
             ...
             <p className="text-stone-500 text-sm mb-6 text-center">Это критически важно: от уровня зависит сложность примеров от ИИ.</p>
             ...
             {deferredPrompt && (
                 <button onClick={()=>{ deferredPrompt.prompt(); deferredPrompt.userChoice.then(()=>{ setDeferredPrompt(null); }) }} className="w-full py-4 bg-orange-50 text-orange-600 font-bold rounded-2xl flex items-center justify-center gap-2 border border-orange-100"><Download className="w-5 h-5"/> Установить на экран "Домой"</button>
             )}
          </div>
        )}
        <div className="p-6 bg-stone-50 border-t border-stone-100 flex gap-3">
          {step > 0 && <button onClick={() => setStep(s=>s-1)} className="px-6 bg-stone-200 text-stone-600 font-bold rounded-2xl">Назад</button>}
          <button onClick={() => { if (step < 4) setStep(s=>s+1); else onSave(level); }} className="flex-1 bg-stone-900 text-white font-bold py-4 rounded-2xl active:scale-95 transition-transform">{step < 4 ? 'Далее' : 'Начать обучение'}</button>
        </div>
      </motion.div>
    </motion.div>
  );
