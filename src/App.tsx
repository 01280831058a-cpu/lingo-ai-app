...
function useTrainingQueue(initialWords: Word[]) {
   ...
   const handleNext = (word: Word, isCorrect: boolean) => {
      if (!isCorrect) setQueue(prev => [...prev, word]);
      setIdx(c => c + 1);
   };
        ...
        <div className="pb-8">
           {status === 'idle' && (
              <button onClick={async () => { 
                 ...
                 disabled={!input} 
                 className="w-full bg-indigo-600 text-white font-bold py-5 rounded-[2rem] flex gap-2 justify-center shadow-[0_8px_20px_rgb(79,70,229,0.2)]"
              >
           ...
           {(status === 'correct' || status === 'incorrect') && (
              <button onClick={() => { 
                 ...
                 className="w-full bg-stone-800 text-white font-bold py-5 rounded-[2rem]"
              >
      ...
      <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} transition={{type:'spring', stiffness: 200, damping: 25}} className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-[0_20px_60px_rgb(0,0,0,0.1)] relative z-10">
        ...
        {step < 4 ? (
          <div className="p-8 text-center">
            <div className="w-16 h-16 mx-auto bg-stone-50 rounded-2xl flex items-center justify-center mb-6">
              {slides[step].icon}
            </div>
          ...
          <div className="p-8">
             ...
             <p className="text-stone-500 text-sm mb-6 text-center">Это критически важно: от уровня зависит лексика и сложность примеров, которые будет генерировать ИИ.</p>
        ...
        <div className="p-6 bg-stone-50 border-t border-stone-100 flex gap-3">
          ...
          <button onClick={() => { if (step < 4) setStep(s=>s+1); else onSave(level); }} className="flex-1 bg-stone-900 text-white font-bold py-4 rounded-2xl active:scale-95 transition-transform">
            {step < 4 ? 'Далее' : 'Начать обучение'}
          </button>
        </div>
      </motion.div>
    </motion.div>
