          ...
          <AnimatePresence>
            ...
            {showBulkAddGroup && (
               <BulkAddGroupModal groups={groups} onClose={() => setShowBulkAddGroup(false)} 
                 onOpenAddGroup={() => {
                   // Открываем модалку создания группы, передаем ей callback на авто-добавление слов
            ...
            {/* Специальная модалка добавления группы, если она вызвана из Bulk-режима (для авто-добавления) */}
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  if (loading) return <div className="flex flex-col items-center justify-center p-8 h-64"><Loader2 className="w-8 h-8 animate-spin text-teal-600 mb-4"/><p className="text-stone-500 font-bold">ИИ готовит викторину...</p></div>;
