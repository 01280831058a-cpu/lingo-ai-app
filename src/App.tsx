        ...
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
          ...
          <AnimatePresence mode="wait">
            {!activeTrainingMode && !viewingGroupId && activeTab === 'dict' && (
              <motion.div key="dict" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-4 md:p-8 space-y-3 pb-32">
            ...
            {!activeTrainingMode && !viewingGroupId && activeTab === 'groups' && (
              <motion.div key="groups" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-4 md:p-8 space-y-3">
            ...
            {!activeTrainingMode && !viewingGroupId && activeTab === 'train' && (
              <motion.div key="train" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 md:p-8 pt-12 md:pt-8">
            ...
            {!activeTrainingMode && !viewingGroupId && activeTab === 'settings' && (
              <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 md:p-8 pt-12 md:pt-8 flex flex-col justify-between h-full">
                <div><h1 className="text-3xl font-bold tracking-tight mb-8 text-stone-800">Настройки</h1>
                  {user && (
                    <div className="bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 mb-6">
                       ...
                       <div className="text-stone-500 text-sm mb-6">{user.email || 'Нет email'}</div>
...
// --- МОДАЛКИ (С ИСПРАВЛЕННЫМ ФОНОМ ЗАТЕМНЕНИЯ) ---
...
function ModeConstructor({ words, onProgress, onFinish }: any) {
   ...
   const handleKeyDown = useCallback((e: KeyboardEvent) => {
      ...
      // Находим первую подходящую букву в пуле
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  if (phase === 'loading') return <div className="flex flex-col items-center justify-center p-8 h-64"><Loader2 className="w-8 h-8 animate-spin text-teal-600 mb-4"/><p className="text-stone-500 font-bold">ИИ готовит викторину...</p></div>;
  if (phase === 'ready') return (
     <div className="flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-stone-500 mb-8">ИИ подготовил уникальные варианты ответов для {words.length} слов.</p>
    ...
    <div className="w-full max-w-sm flex flex-col">
       ...
       <div className="space-y-3">
          {currentItem.options.map((opt: string, i: number) => {
             ...
             return <button key={i} onClick={() => { if(ansIdx===null) { setAnsIdx(i); onProgress(currentItem.id, opt === currentItem.translation); setTimeout(() => { if(currentIndex >= words.length-1) onFinish(); else {setCurrentIndex(c=>c+1); setAnsIdx(null);} }, 1500) } }} className={`w-full p-5 rounded-2xl border font-bold text-lg active:scale-[0.98] transition-all shadow-sm ${stateClass}`}>{opt}</button>;
          })}
       </div>
    </div>
  );
