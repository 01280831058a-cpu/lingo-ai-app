...
import {
  ...
  ChevronLeft, ChevronRight, UploadCloud, Sparkles
} from 'lucide-react';
...
class ApiClient {
  ...
  static async aiGenerateWordsList(topic: string, count: number, text?: string): Promise<any[]> {
     try {
       const res = await fetch(`${this.BASE_URL}/ai`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'generate_words', topic, count, text }) });
       if (!res.ok) throw new Error('API Error'); return await res.json();
     } catch(e) { return []; }
  }
}
...
// Динамический загрузчик PDF.js (без npm install)
                 ...
                 <motion.div key="tab-dict" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8 space-y-3 pb-32 md:pb-8">
                   ...
                   {selectedWordIds.size === 0 && (
                      <>
                         ...
                         <button onClick={() => setShowGenerateModal(true)} className="fixed bottom-40 md:bottom-24 right-5 md:right-8 w-14 h-14 bg-purple-600 text-white rounded-full shadow-[0_8px_30px_rgb(147,51,234,0.3)] flex items-center justify-center active:scale-90 transition-all z-20"><Wand2 className="w-6 h-6" /></button>
                      </>
                   )}
...
// --- МОДАЛКИ ГЕНЕРАЦИИ И ДОБАВЛЕНИЯ ---
function GenerateWordsModal({ words, userProfile, onClose, onSaveWord, onWordClick, user }: any) {
  ...
  const handleGenerate = async () => {
     ...
     setLoading(true);
     const result = await ApiClient.aiGenerateWordsList(topic, c, extractedText);
                ...
                {activeTab.generatedWords.length === 0 ? (
                   <>
                      <div className="grid grid-cols-3 gap-3">
                         <div className="col-span-2 space-y-1"><label className="text-[10px] font-bold text-stone-400 uppercase">Тема (или оставьте пустым для файла)</label><input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Напр. Путешествия" className="w-full bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold" /></div>
                         <div className="col-span-1 space-y-1"><label className="text-[10px] font-bold text-stone-400 uppercase">Кол-во слов</label><input type="number" min="1" max="50" value={count} onChange={e => setCount(e.target.value)} className="w-full bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold" /></div>
                      </div>
                      <div className="space-y-1">
                         <label className="text-[10px] font-bold text-stone-400 uppercase block">Или извлечь из файла (PDF/TXT)</label>
                   ...
                   <div>
                      ...
                      <div className="space-y-2">
                         {activeTab.generatedWords.map((w:any, i:number) => {
                            const alreadyInTabs = tabs.some(t => t.id !== 'generator' && (t.wordData?.original || t.original).toLowerCase() === w.word.toLowerCase());
                            const existingInDict = words.find((x:any) => x.original.toLowerCase() === w.word.toLowerCase());
                            return (
                               <div key={i} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-stone-200 shadow-sm">
                                  <div><span className="font-bold text-stone-800 text-lg">{w.word.toLowerCase()}</span><div className="text-sm text-stone-500">{w.translation.toLowerCase()}</div></div>
                                  {existingInDict ? (
...
function AddWordModal({ words, userProfile, groups, onClose, onSaveWord, onWordClick, user }: any) {
  ...
  const handleAddRelated = async (rw: any) => {
     const newId = Date.now().toString();
     setTabs(prev => [...prev, { id: newId, original: rw.word.toLowerCase(), status: 'analyzing', wordData: {}, selectedTranslation: '', groupIds: new Set() }]);
     setActiveTabId(newId);
     const result = await ApiClient.aiGenerateWord(rw.word.toLowerCase(), userProfile?.level);
                           ...
                           {relatedList.map((rw:any, i:number) => {
                              ...
                              return (
                                 <div key={i} className="flex items-center justify-between bg-white p-3 rounded-xl border border-stone-200 shadow-sm">
                                    <div className="text-left"><span className="font-bold text-stone-800">{rw.word.toLowerCase()}</span> <span className="text-[10px] text-stone-400 bg-stone-100 px-1.5 py-0.5 rounded ml-1 uppercase">{rw.partOfSpeech}</span><div className="text-xs text-stone-500 mt-1">{rw.translation.toLowerCase()}</div></div>
...
function WordEditorModal({ word, words, groups, userProfile, user, onClose, onSave, onDelete, onReset, onWordClick }: any) {
  ...
  const handleAddRelated = async (rw: any) => {
     ...
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: rw.word.toLowerCase(), translation: (result.translationOptions?.[0] || result.translation || rw.translation).toLowerCase(), createdAt: Date.now(), masteryLevel: 0, lastPracticed: Date.now() });
           ...
           {relatedTabs.length > 0 && (
              <div className="flex overflow-x-auto gap-2 pb-2 mb-4 hide-scrollbar border-b border-stone-100">
                  ...
                  {relatedTabs.map((rw: any, i: number) => {
                      ...
                      if (existing) {
                           return <button key={i} onClick={() => onWordClick(existing.id)} className="px-4 py-2 bg-stone-100 text-stone-600 rounded-xl text-sm font-bold shrink-0 active:scale-95 transition-transform">{rw.word.toLowerCase()}</button>
                      } else {
                           return <button key={i} onClick={() => handleAddRelated(rw)} disabled={loadingRelated === rw.word} className="px-4 py-2 bg-white border border-stone-200 border-dashed text-stone-400 rounded-xl text-sm font-bold shrink-0 flex items-center gap-1 active:scale-95 transition-transform disabled:opacity-50">{loadingRelated === rw.word ? <Loader2 className="w-3 h-3 animate-spin"/> : <Plus className="w-3 h-3"/>} {rw.word.toLowerCase()}</button>
                      }
                  })}
              </div>
           )}
