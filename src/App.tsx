...
import {
  ...
  Volume2, VolumeX, ArrowDownAZ, ArrowUpAZ, Clock, Tag, Mic, Square
} from 'lucide-react';
...
class ApiClient {
  ...
  // НОВЫЙ МЕТОД ДЛЯ АНАЛИЗА РЕЧИ
  static async aiAnalyzeSpeech(audioBlob: Blob, topic: string, level?: string): Promise<{ transcript: string, feedback: string }> {
    try {
           ...
           reader.onloadend = async () => {
               ...
               try {
                   const res = await fetch(`${this.BASE_URL}/ai`, { 
                       ...
                       body: JSON.stringify({ action: 'analyze_speech', audio: base64Audio, topic, level }) 
                   });
                   if (!res.ok) throw new Error('API Error');
       ...
       return { transcript: '', feedback: 'Ошибка анализа ИИ. Проверьте подключение.' }; 
    }
  }
}
...
function AuthScreen() {
    ...
    {alertData && <AlertModal title={alertData.title} message={alertData.message} type={alertData.type} onClose={() => setAlertData(null)} />}
           <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
               ...
               <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2rem] p-6 shadow-2xl relative z-10 w-full max-w-sm text-center">
                  ...
                  <p className="text-stone-500 text-sm mb-6">Хотите создать новый аккаунт с этим email?</p>
                  <div className="flex flex-col gap-3">
                      ...
                      <button onClick={() => setShowRegisterPrompt(false)} className="w-full py-4 bg-stone-100 text-stone-700 font-bold rounded-2xl active:scale-95 transition-transform">Отмена</button>
                  </div>
               </motion.div>
           </div>
        )}
   ...
   <img src="/frog.png" alt="Фрогги" className="w-24 h-24 object-contain mb-4 drop-shadow-md" />
   <h1 className="text-4xl font-black mb-2 text-center text-stone-800 tracking-tight">Фрогги</h1>
   <p className="text-stone-500 text-center mb-8 font-bold text-sm">Словарь</p>
           ...
           <div className="w-full flex justify-between items-center mt-4 text-sm font-bold">
              ...
              ) : (<button onClick={()=>setView('login')} className="text-stone-500 mx-auto w-full text-center">Вернуться</button>)}
           </div>
           {view !== 'forgot' && (
...
function MainApp({ user }: { user: User }) {
  ...
  const [activeTrainingMode, setActiveTrainingMode] = useState<'flashcards'|'quiz'|'sentence'|'constructor'|'brainstorm'|'speaking'|'stats'|null>(null);
  ...
  const resetAllProgress = () => { setAppConfirm({title:"Сброс прогресса", message:"Сбросить весь прогресс?", onConfirm: () => { words.forEach(w => updateDoc(doc(db, 'users', user.uid, 'words', w.id), { correctAnswers: 0, incorrectAnswers: 0, masteryLevel: 0, lastPracticed: Date.now() })); }}); };
  const resetEntireDictionary = async () => { setAppConfirm({title:"Очистка словаря", message:"Это удалит ВСЕ слова и группы навсегда! Уверены?", onConfirm: async () => { words.forEach(w => deleteDoc(doc(db, 'users', user.uid, 'words', w.id))); groups.forEach(g => deleteDoc(doc(db, 'users', user.uid, 'groups', g.id))); await updateDoc(doc(db, 'users', user.uid, 'profile', 'data'), { activity: {}, streak: 0 }); }}); };
  ...
  const handleUpdateProgress = (wordId: string, isCorrect: boolean, mode: string = 'general') => {
    ...
    let newMastery = effectiveMastery + (isCorrect ? 5 : -50);
  ...
  if (userProfile && !userProfile.onboarded) return <OnboardingModal user="{user}" onSave="{(level:" string, goal:> setDoc(doc(db, 'users', user.uid, 'profile', 'data'), { level, dailyGoal: goal, onboarded: true, activity: {}, streak: 0 }, { merge: true })} />;
  ...
  <div className="flex items-center gap-3 mb-10 px-2 mt-4">
     <img src="/frog.png" alt="Лого" className="w-10 h-10 object-contain drop-shadow-sm" /> 
     <span className="text-2xl font-black text-stone-800 tracking-tight">Фрогги</span>
  </div>
      ...
      {!activeTrainingMode && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-white/70 backdrop-blur-xl border-t border-white/60 flex justify-around items-center px-2 z-40 pb-safe shadow-[0_-8px_30px_rgb(0,0,0,0.02)]">
          ...
          <NavItem active="{activeTab" 'groups'} icon="{<Layers" className="w-6 h-6"/>} label="Группы" onClick={() => { setActiveTab('groups'); setViewingGroupId(null); setGroupSearch(''); }} />
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-y-auto ${!activeTrainingMode ? 'md:ml-64 pb-24 md:pb-0' : ''}`}>
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
            ...
            <AnimatePresence mode="wait">
                   ...
                   {words.length > 0 && (
                      <>
                         <div className="relative mb-2">
                            ...
                            <input type="text" placeholder="Поиск по словарю..." value={dictSearch} onChange={e => setDictSearch(e.target.value)} className="w-full bg-white pl-12 pr-4 py-4 rounded-2xl border border-stone-200 outline-none focus:border-teal-500 transition-colors shadow-sm font-medium" />
                         </div>
                         <div className="flex gap-2 mb-4 overflow-x-auto hide-scrollbar pb-1">
                            <button onClick={()=>setSortOrder(s => s === 'newest' ? 'oldest' : 'newest')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder.includes('est') ? 'bg-stone-800 text-white' : 'bg-stone-200 text-stone-600 active:scale-95'}`}><Clock className="w-3 h-3"/> {sortOrder === 'oldest' ? 'Старые' : 'Новые'}</button>
                            <button onClick={()=>setSortOrder(s => s === 'alphaAsc' ? 'alphaDesc' : 'alphaAsc')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder.includes('alpha') ? 'bg-stone-800 text-white' : 'bg-stone-200 text-stone-600 active:scale-95'}`}>{sortOrder === 'alphaDesc' ? <ArrowUpAZ className="w-3 h-3"/> : <ArrowDownAZ className="w-3 h-3"/>} {sortOrder === 'alphaDesc' ? 'Я-А' : 'А-Я'}</button>
                            <button onClick={()=>setSortOrder('pos')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder==='pos'?'bg-stone-800 text-white':'bg-stone-200 text-stone-600 active:scale-95'}`}><Tag className="w-3 h-3"/> Часть речи</button>
                         </div>
                      </>
                   )}
                   ...
                   {words.length > 0 && (
                      <div className="flex justify-between items-center px-1 mb-2">
                         ...
                         <button onClick={() => {
                            ...
                            {sortedDictWords.length > 0 && sortedDictWords.every(w => selectedWordIds.has(w.id)) ? 'Снять выделение' : 'Выбрать все'}
                         </button>
                      </div>
                   )}

                   {words.length === 0 ? <div className="text-center text-stone-400 py-12">Словарь пуст.</div> : sortedDictWords.length === 0 ? <div className="text-center text-stone-400 py-12">По вашему запросу ничего не найдено.</div> : sortedDictWords.map(word => (
                     <div key={word.id} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 active:scale-[0.98] transition-transform clickable-card cursor-pointer" onClick={() => setViewingWordId(word.id)}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                             <h3 className="text-lg font-bold text-stone-800 truncate">{(word.original || '').toLowerCase()}</h3> 
                             {word.partOfSpeech && <span className="text-[10px] font-bold text-stone-400 uppercase bg-stone-100 px-2 py-0.5 rounded-md shrink-0">{word.partOfSpeech}</span>}
                          </div>
                          <p className="text-stone-500 text-sm mt-0.5 truncate">{(word.translation || '').toLowerCase()}</p>
                          <MasteryBar masteryLevel="{getEffectiveMastery(word)}"/>
                        </div>
                        
                        <button onClick={(e) => { e.stopPropagation(); ApiClient.playTTS(word.original); }} className="p-2 bg-stone-50 text-teal-600 rounded-full shrink-0 active:scale-90 hover:bg-stone-100"><Volume2 className="w-5 h-5"/></button>
                     </div>
                   ))}
                 ...
                 <motion.div key="tab-groups" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8 space-y-3 pb-32 md:pb-8">
                    ...
                    {(groups || []).length > 0 && (
                      <div className="relative mb-4">
                         <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-stone-400"/>
                         <input type="text" placeholder="Поиск групп..." value={groupSearch} onChange={e => setGroupSearch(e.target.value)} className="w-full bg-white pl-12 pr-4 py-4 rounded-2xl border border-stone-200 outline-none focus:border-teal-500 transition-colors shadow-sm font-medium" />
                      </div>
                    )}

                    {(groups || []).length === 0 ? <div className="text-center text-stone-400 py-12">Нет групп.</div> : filteredGroups.length === 0 ? <div className="text-center text-stone-400 py-12">Группы не найдены.</div> : filteredGroups.map(group => (
                      <div key={group.id} onClick={() => setViewingGroupId(group.id)} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 cursor-pointer active:scale-[0.98] transition-transform clickable-card">
                          ...
                          <div className="flex-1 min-w-0"><h3 className="text-lg font-bold text-stone-800 truncate">{group.name}</h3><p className="text-stone-500 text-sm mt-0.5">{(words||[]).filter(w=>(w.groupIds||[]).includes(group.id)).length} слов</p></div>
                   ...
                   {(selectedWordIds.size === 0 && selectedGroupIds.size === 0) ? (
                      <div className="mb-8">
                         ...
                         <div className="space-y-3">
                            <button onClick={() => setActiveTrainingMode('speaking')} className="w-full bg-rose-50 border border-rose-100 text-rose-700 p-5 rounded-[2rem] font-bold text-left active:scale-[0.98] transition-transform flex items-center justify-between mb-3">
                               <span className="flex items-center gap-2"><Mic className="w-5 h-5"/> Минута речи (Спикинг)</span> <span className="font-normal opacity-70">Без слов</span>
                            </button>
               ...
               {!activeTrainingMode && !viewingGroupId && activeTab === 'settings' && (
                 <motion.div key="tab-settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8 pb-32 md:pb-8">
                     ...
                     <h3 className="font-bold text-stone-800 mb-4 mt-6">Оформление и звук</h3>
                     <button onClick={() => { 
                     ...
                     }} className="w-full bg-stone-50 p-4 rounded-2xl flex justify-between items-center font-bold text-stone-700 active:scale-95 mb-3 border border-stone-100">
                         <span className="flex items-center gap-2">{soundEnabled ? <Volume2 className="w-5 h-5 text-teal-600"/> : <VolumeX className="w-5 h-5 text-stone-400"/>} Звуковые эффекты</span>
                         <span className={soundEnabled ? 'text-teal-600' : 'text-stone-400'}>{soundEnabled ? 'Вкл' : 'Выкл'}</span>
                     </button>
                     <button onClick={() => { 
                     ...
                     }} className="w-full bg-stone-50 p-4 rounded-2xl flex justify-between items-center font-bold text-stone-700 active:scale-95 mb-6 border border-stone-100">
                         <span className="flex items-center gap-2">{autoTts ? <Volume2 className="w-5 h-5 text-teal-600"/> : <VolumeX className="w-5 h-5 text-stone-400"/>} Автоозвучка карточек</span>
                         <span className={autoTts ? 'text-teal-600' : 'text-stone-400'}>{autoTts ? 'Вкл' : 'Выкл'}</span>
                     </button>

                     <h3 className="font-bold text-stone-800 mb-4 mt-6">Опасная зона</h3>
                     <div className="space-y-3">
          ...
          </DeleteAccountModal></GroupView></SmartSelectionModal></BulkAddGroupModal></AddGroupModal></WordEditorModal></WordViewModal></AddWordModal></GenerateWordsModal></AnimatePresence>
        </div>
      </main>
               ...
               <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center justify-center">
                  {activeTrainingMode === 'stats' ? <SessionStats stats="{sessionStats}" onClose="{closeTraining}"/> : (
                     <div className="w-full max-w-sm">
                        ...
                        {activeTrainingMode === 'sentence' && <ModeSentence words="{trainingSnapshot}" onProgress="{(w:" string, c:> handleUpdateProgress(w, c, 'sentence')} onFinish={() => setActiveTrainingMode('stats')} />}
                        ...
                        {activeTrainingMode === 'speaking' && <ModeSpeaking onFinish="{()"> setActiveTrainingMode(null)} userProfile={userProfile} />}
                     </div>
                  )}
               </div>
            </motion.div>
         )}
      </ModeSpeaking></ModeBrainstorm></ModeSentence></ModeConstructor></ModeQuiz></ModeFlashcards></AnimatePresence>
    </div>
  );
}
      ...
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative z-10">
           ...
           <div className="bg-stone-50 p-4 rounded-2xl mb-6 border border-stone-200">
              <div className="flex justify-between text-xs font-bold text-stone-400 mb-2 uppercase"><span>Количество</span><span>Всего: {safeWords.length}</span></div>
              <input 
                 type="number" 
                 value={inputVal} 
...
function GenerateWordsModal({ words, userProfile, groups, onClose, onSaveWord, onWordClick }: any) {
  ...
  const handleCloseTab = (id: string, e?: React.MouseEvent) => {
     ...
     setTabs(newTabs); 
     if(activeTabId === id) {
         const nextWordTab = newTabs.find(t => t.type === 'word');
         if (nextWordTab) setActiveTabId(nextWordTab.id);
         else setActiveTabId('generator');
     }
  };

  const handleReset = () => {
      updateWordTab('generator', {generatedWords: []});
      setTopic('');
      setExtractedText('');
      setFileName('');
      setCount('10');
  ...
  const toggleSelect = (word: string) => {
      const newSet = new Set(selectedGenWords);
      if(newSet.has(word)) newSet.delete(word);
      else newSet.add(word);
      setSelectedGenWords(newSet);
  };

  const selectAll = () => {
      if (selectedGenWords.size === (activeTab.generatedWords || []).length) setSelectedGenWords(new Set());
      else setSelectedGenWords(new Set((activeTab.generatedWords || []).map((w:any) => w.word)));
  };
  ...
  const handleAddSelectedToTabs = async () => {
      ...
      const newTabs = wordsToAdd.map((w:any, i:number) => ({
          id: Date.now().toString() + i,
          type: 'word',
          original: (w.word || '').toLowerCase(),
          status: 'analyzing',
          wordData: {},
          selectedTranslation: '',
          groupIds: new Set()
      ...
      await Promise.all(newTabs.map(async (tab: any, i: number) => {
          ...
          setTabs(prev => prev.map(t => t.id === tab.id ? { 
              ...
              status: 'done', 
              wordData: result, 
              selectedTranslation: (result.translationOptions?.[0] || result.translation || w.translation || '').toLowerCase() 
          } : t));
      }));
    ...
    <div className="fixed inset-0 z-[60] flex flex-col justify-end items-center">
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
        ...
        <div className="flex overflow-x-auto gap-2 pb-2 mb-4 hide-scrollbar border-b border-stone-100">
           {tabs.map((t) => (
               <div key={t.id} onClick={() => setActiveTabId(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-bold shrink-0 cursor-pointer transition-all border-b-2 clickable-card ${activeTabId === t.id ? 'bg-purple-50 text-purple-700 border-purple-500' : 'bg-transparent text-stone-400 border-transparent hover:bg-stone-50'}`}>
                ...
                {(activeTab.generatedWords || []).length === 0 ? (
                   <>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                         <div className="col-span-2 space-y-1">
                             <label className="text-[10px] font-bold text-stone-400 uppercase">Тема (или пусто для PDF)</label>
                             <textarea 
                                 value={topic} 
                                 onChange={e => {
                                 ...
                                 placeholder="Напр. Путешествия" 
                                 className="w-full bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold resize-none overflow-hidden" 
                                 rows={1}
                   ...
                   <div className="pb-24">
                      <div className="flex justify-between items-center mb-4">
                         <button onClick={selectAll} className="text-xs font-bold text-purple-600 flex items-center gap-1"><CheckCircle2 className="w-4 h-4"/> {selectedGenWords.size === (activeTab.generatedWords || []).length ? 'Снять все' : 'Выбрать все'}</button>
                         <button onClick={handleReset} className="text-xs font-bold text-stone-400">Сбросить</button>
                      </div>
                      <div className="space-y-2">
                         {(activeTab.generatedWords || []).map((w:any, i:number) => {
                            ...
                            return (
                               <div key={i} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-stone-200 shadow-sm gap-3">
                                  <button onClick={() => toggleSelect(w.word)} className={`w-6 h-6 rounded-full border-2 flex shrink-0 items-center justify-center transition-colors ${selectedGenWords.has(w.word) ? 'bg-purple-500 border-purple-500' : 'border-stone-300'}`}>
                                      {selectedGenWords.has(w.word) && <Check className="w-4 h-4 text-white"/>}
                                  </button>
                                  <div className="flex-1 min-w-0 cursor-pointer clickable-card" onClick={() => toggleSelect(w.word)}>
                                     <div className="font-bold text-stone-800 text-lg truncate">{cleanWWord}</div>
                                     <div className="text-sm text-stone-500 truncate">{(w.translation || '').toLowerCase()}</div>
                                  </div>
                                  {existingInDict ? (
                                     <button onClick={() => onWordClick(existingInDict.id)} className="px-3 py-1.5 bg-stone-100 text-stone-600 rounded-xl text-xs font-bold shrink-0">В словаре</button>
                                  ) : (
                      ...
                      <AnimatePresence>
                         {selectedGenWords.size > 0 && (
                            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm bg-white p-2 rounded-[2rem] shadow-2xl flex items-center z-50 border border-stone-100">
                               <button onClick={handleAddSelectedToTabs} disabled={loadingBatch} className="w-full py-4 bg-purple-600 text-white font-bold rounded-2xl active:scale-95 transition-transform flex justify-center items-center gap-2">
                                  {loadingBatch ? <Loader2 className="w-5 h-5 animate-spin"/> : `Добавить во вкладки (${selectedGenWords.size})`}
                               </button>
                            </motion.div>
                         )}
                      </AnimatePresence>
             ...
             <>
                ...
                {activeTab.status === 'done' && (
                   activeTab.wordData?.translationOptions?.[0]?.includes('Ошибка ИИ') ? (
                      <div className="bg-rose-50 p-6 rounded-[2rem] border border-rose-100 text-center mt-4">
                         <AlertTriangle className="w-12 h-12 text-rose-500 mx-auto mb-4"/>
                         <h3 className="text-xl font-bold text-stone-800 mb-2">Сбой генерации</h3>
                         <p className="text-stone-500 text-sm mb-6">Не удалось получить данные от ИИ. Возможно, вы ввели непонятное слово, или Яндекс перегружен.</p>
                         <button onClick={handleAnalyzeWord} className="w-full py-4 bg-rose-500 text-white font-bold rounded-2xl active:scale-95 transition-transform flex items-center justify-center gap-2"><RefreshCw className="w-5 h-5"/> Попробовать еще раз</button>
                      </div>
                   ) : (
                      <div className="space-y-4">
                         <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                            <div className="flex flex-col items-center mb-4">
                               <div className="flex items-center gap-3 mb-2">
                                  {activeTab.wordData?.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-widest">{activeTab.wordData.partOfSpeech}</span>}
                                  <button onClick={() => ApiClient.playTTS(activeTab.wordData?.original || activeTab.original)} className="w-10 h-10 flex items-center justify-center bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200 active:scale-90 transition-all shrink-0">
                                     <Volume2 className="w-5 h-5"/>
                                  </button>
                               </div>
                               <h3 className="text-3xl font-black text-stone-800 text-center break-words w-full leading-tight">{(activeTab.wordData?.original || activeTab.original || '').toLowerCase()}</h3>
                            </div>
                            ...
                            {(activeTab.wordData?.examples || []).map((ex:any, i:number) => <div key={i} className="mt-6 p-5 bg-purple-50 border border-purple-100 rounded-2xl text-center"><div className="text-[10px] font-bold text-purple-600/70 uppercase mb-2">Пример ИИ</div><div className="font-medium text-purple-900 mb-2 text-lg">"{ex.text}"</div><div className="text-sm text-purple-700/80">{(ex.translation || '').toLowerCase()}</div></div>)}
                            
                            {(activeTab.wordData?.relatedWords || []).length > 0 && (
                               <div className="mt-6 pt-4 border-t border-stone-200/50 text-center">
                                  ...
                                  <div className="space-y-2">
                                     {(activeTab.wordData?.relatedWords || []).map((rw:any, i:number) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col justify-end items-center">
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
        ...
        <div className="flex overflow-x-auto gap-2 pb-2 mb-4 hide-scrollbar border-b border-stone-100">
           {tabs.map((t) => (
               <div key={t.id} onClick={() => setActiveTabId(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-bold shrink-0 cursor-pointer transition-all border-b-2 clickable-card ${activeTabId === t.id ? 'bg-teal-50 text-teal-700 border-teal-500' : 'bg-transparent text-stone-400 border-transparent hover:bg-stone-50'}`}>
          ...
          {activeTab.status === 'done' && (
             activeTab.wordData?.translationOptions?.[0]?.includes('Ошибка ИИ') ? (
                <div className="bg-rose-50 p-6 rounded-[2rem] border border-rose-100 text-center mt-4">
                   ...
                   <h3 className="text-xl font-bold text-stone-800 mb-2">Сбой генерации</h3>
                   <p className="text-stone-500 text-sm mb-6">Не удалось получить данные от ИИ. Возможно, вы ввели непонятное слово, или Яндекс перегружен.</p>
                   <button onClick={handleAnalyze} className="w-full py-4 bg-rose-500 text-white font-bold rounded-2xl active:scale-95 transition-transform flex items-center justify-center gap-2"><RefreshCw className="w-5 h-5"/> Попробовать еще раз</button>
                </div>
             ) : (
                <div className="space-y-4">
                   <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                      <div className="flex flex-col items-center mb-4">
                         <div className="flex items-center gap-3 mb-2">
                            {activeTab.wordData?.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-widest">{activeTab.wordData.partOfSpeech}</span>}
                            <button onClick={() => ApiClient.playTTS(activeTab.wordData?.original || activeTab.original)} className="w-10 h-10 flex items-center justify-center bg-teal-100 text-teal-700 rounded-full hover:bg-teal-200 active:scale-90 transition-all shrink-0">
                               <Volume2 className="w-5 h-5"/>
                            </button>
                         </div>
                         <h3 className="text-3xl font-black text-stone-800 text-center break-words w-full leading-tight">{(activeTab.wordData?.original || activeTab.original || '').toLowerCase()}</h3>
                      ...
                      {(activeTab.wordData?.examples || []).map((ex:any, i:number) => <div key={i} className="mt-6 p-5 bg-teal-50 border border-teal-100 rounded-2xl text-center"><div className="text-[10px] font-bold text-teal-600/70 uppercase mb-2">Пример ИИ</div><div className="font-medium text-teal-900 mb-2 text-lg">"{ex.text}"</div><div className="text-sm text-teal-700/80">{(ex.translation || '').toLowerCase()}</div></div>)}
...
// НОВОЕ ОКНО: РЕЖИМ ПРОСМОТРА КАРТОЧКИ
     ...
     <div className="fixed inset-0 z-[100] flex flex-col justify-end items-center">
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           ...
           <div className="flex justify-between items-center mb-6 shrink-0">
              <h2 className="text-xl font-bold text-stone-800 flex items-center gap-2"><BookOpen className="w-5 h-5 text-teal-600"/> Карточка слова</h2>
              <div className="flex items-center gap-2">
                 <button onClick={onEdit} className="p-2 bg-stone-100 text-stone-600 rounded-full active:scale-90 transition-transform"><Edit3 className="w-5 h-5"/></button>
           ...
           <div className="flex-1 overflow-y-auto space-y-4 pb-6 hide-scrollbar">
              <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                 <div className="flex flex-col items-center mb-4">
                    <div className="flex items-center gap-3 mb-2">
                       {word.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-widest">{word.partOfSpeech}</span>}
                       <button onClick={() => ApiClient.playTTS(word.original)} className="w-10 h-10 flex items-center justify-center bg-teal-100 text-teal-700 rounded-full hover:bg-teal-200 active:scale-90 transition-all shrink-0">
                          <Volume2 className="w-5 h-5"/>
                       </button>
                    </div>
                    <h3 className="text-3xl font-black text-stone-800 text-center break-words w-full leading-tight">{(word.original || '').toLowerCase()}</h3>
                 </div>

                 <div className="text-center mb-6">
                    <div className="text-xl font-bold text-teal-600">{(word.translation || '').toLowerCase()}</div>
                 </div>

                 {word.cambridgeTranslation && (
                    <div className="pt-4 border-t border-stone-200/50">
                       <div className="text-[10px] font-bold text-stone-400 uppercase text-center mb-1">Cambridge Dictionary</div>
                       <div className="text-stone-600 text-center text-sm leading-relaxed">{word.cambridgeTranslation}</div>
                    </div>
                 )}

                 {(word.transcriptionUK || word.transcriptionUS) && (
                    <div className="pt-4 border-t border-stone-200/50">
                       <div className="text-[10px] font-bold text-stone-400 uppercase text-center mb-2">Транскрипция</div>
                       <div className="flex justify-center gap-4 text-sm font-medium text-stone-500">
                          {word.transcriptionUK && <span className="bg-white px-3 py-1 rounded-lg border border-stone-200 shadow-sm">UK: [{word.transcriptionUK}]</span>}
                          {word.transcriptionUS && <span className="bg-white px-3 py-1 rounded-lg border border-stone-200 shadow-sm">US: [{word.transcriptionUS}]</span>}
                       </div>
                    </div>
                 )}

                 {(word.examples || []).map((ex:any, i:number) => (
                    <div key={i} className="mt-6 p-5 bg-teal-50 border border-teal-100 rounded-2xl text-center">
                       <div className="text-[10px] font-bold text-teal-600/70 uppercase mb-2">Пример</div>
                       <div className="font-medium text-teal-900 mb-2 text-lg">"{ex.text}"</div>
                       <div className="text-sm text-teal-700/80">{(ex.translation || '').toLowerCase()}</div>
                    </div>
                 ))}

                 {relatedTabs.length > 0 && (
                    <div className="mt-6 pt-4 border-t border-stone-200/50 text-center">
                       <div className="text-[10px] font-bold text-stone-400 uppercase mb-3">Однокоренные слова</div>
                       <div className="flex flex-wrap justify-center gap-2">
                          {relatedTabs.map((rw: any, i: number) => {
                             const cleanRW = (rw.word || '').toLowerCase();
                             const existing = (words||[]).find((w:any) => (w.original || '').toLowerCase() === cleanRW);
                             if (existing) {
                                return <button key={i} onClick={() => onWordClick(existing.id)} className="px-3 py-1.5 bg-stone-200 text-stone-700 rounded-xl text-xs font-bold active:scale-95 transition-colors">{cleanRW}</button>;
                             }
                             return <span key={i} className="px-3 py-1.5 bg-white border border-stone-200 text-stone-400 rounded-xl text-xs font-bold">{cleanRW}</span>;
                          })}
                       </div>
                    </div>
                 )}
                 ...
                 {(word.groupIds || []).length > 0 && (
                    <div className="mt-6 pt-4 border-t border-stone-200/50 text-center">
                       <div className="text-[10px] font-bold text-stone-400 uppercase mb-3">Группы</div>
                       <div className="flex flex-wrap justify-center gap-2">
                          {(word.groupIds || []).map((gid: string) => {
                             const g = (groups || []).find((x:Group) => x.id === gid);
                             return g ? <span key={gid} className="px-3 py-1.5 bg-sky-50 text-sky-700 border border-sky-100 rounded-xl text-xs font-bold">{g.name}</span> : null;
                          })}
                       </div>
                    </div>
                 )}
              ...
              <button onClick={onEdit} className="w-full py-5 bg-teal-600 text-white font-bold rounded-[2rem] active:scale-95 transition-transform flex justify-center items-center gap-2">
                 <Edit3 className="w-5 h-5"/> Редактировать карточку
              </button>
           </div>
        </motion.div>
     </div>
  ...
  return (
     <div className="fixed inset-0 z-[100] flex flex-col justify-end items-center">
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           ...
           <button onClick={handleSave} className="w-full py-5 bg-teal-600 text-white font-bold rounded-[2rem] active:scale-95 transition-colors text-lg shrink-0">Сохранить изменения</button>
        </motion.div>
     </div>
  );
}
...
function GroupView({ sortOrder, setSortOrder, group, words, onClose, onDeleteGroup, onRemoveFromGroup, selectedWordIds, setSelectedWordIds, onTrain, onWordClick }: any) {
   const [search, setSearch] = useState('');
   const [isEditingTitle, setIsEditingTitle] = useState(false);
   const [editTitle, setEditTitle] = useState(group.name);

   const saveTitle = () => {
      if (editTitle.trim() && editTitle.trim() !== group.name) {
         updateDoc(doc(db, 'users', auth.currentUser!.uid, 'groups', group.id), { name: editTitle.trim() });
      }
      setIsEditingTitle(false);
   };

   let sortedGroupWords = (words||[]).filter((w: Word) => (w.original || '').toLowerCase().includes(search.toLowerCase()) || (w.translation || '').toLowerCase().includes(search.toLowerCase()));
   sortedGroupWords.sort((a: Word, b: Word) => {
       if (sortOrder === 'alphaAsc') return (a.original||'').localeCompare(b.original||'');
       if (sortOrder === 'alphaDesc') return (b.original||'').localeCompare(a.original||'');
       if (sortOrder === 'pos') return (a.partOfSpeech||'z').localeCompare(b.partOfSpeech||'z');
       if (sortOrder === 'oldest') return (a.createdAt||0) - (b.createdAt||0);
   ...
   return (
      <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }} className="absolute inset-0 z-20 bg-[#F7F7F5] flex flex-col pt-12 md:pt-8 overflow-hidden">
         <div className="flex items-center px-4 md:px-8 pb-4 border-b border-stone-200/50 sticky top-0 z-10 bg-[#F7F7F5]/80 backdrop-blur-xl">
            ...
            <div className="flex-1 ml-2 min-w-0">
               {isEditingTitle ? (
                  <input autoFocus value={editTitle} onChange={e=>setEditTitle(e.target.value)} onBlur={saveTitle} onKeyDown={e=>e.key==='Enter'&&saveTitle()} className="text-xl font-bold bg-white border border-stone-200 rounded px-2 py-1 outline-none w-full" />
               ) : (
                  <h2 className="text-xl font-bold text-stone-800 flex items-center gap-2 cursor-pointer truncate" onClick={()=>setIsEditingTitle(true)}>{group.name} <Edit3 className="w-4 h-4 text-stone-400 shrink-0"/></h2>
               )}
         ...
         <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-3 pb-40">
            
            {(words||[]).length > 0 && (
               <>
                  <div className="relative mb-2">
                     <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-stone-400"/>
                     <input type="text" placeholder="Поиск в группе..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white pl-12 pr-4 py-4 rounded-2xl border border-stone-200 outline-none focus:border-teal-500 transition-colors shadow-sm font-medium" />
                  </div>
                  <div className="flex gap-2 mb-4 overflow-x-auto hide-scrollbar pb-1">
                     <button onClick={()=>setSortOrder((s:any) => s === 'newest' ? 'oldest' : 'newest')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder.includes('est')?'bg-stone-800 text-white':'bg-stone-200 text-stone-600 active:scale-95'}`}><Clock className="w-3 h-3"/> {sortOrder === 'oldest' ? 'Старые' : 'Новые'}</button>
                     <button onClick={()=>setSortOrder((s:any) => s === 'alphaAsc' ? 'alphaDesc' : 'alphaAsc')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder.includes('alpha')?'bg-stone-800 text-white':'bg-stone-200 text-stone-600 active:scale-95'}`}>{sortOrder === 'alphaDesc' ? <ArrowUpAZ className="w-3 h-3"/> : <ArrowDownAZ className="w-3 h-3"/>} {sortOrder === 'alphaDesc' ? 'Я-А' : 'А-Я'}</button>
                     <button onClick={()=>setSortOrder('pos')} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 shrink-0 ${sortOrder==='pos'?'bg-stone-800 text-white':'bg-stone-200 text-stone-600 active:scale-95'}`}><Tag className="w-3 h-3"/> Часть речи</button>
                  </div>
               </>
            )}
            ...
            {(words||[]).length > 0 && (
               <div className="flex justify-between items-center px-1 mb-2">
                  <button onClick={() => { 
                     const groupWordIds = sortedGroupWords.map((w: Word) => w.id); 
                     ...
                     <CheckCircle2 className="w-4 h-4"/> 
                     {sortedGroupWords.length > 0 && sortedGroupWords.every((w: Word) => selectedWordIds.has(w.id)) ? 'Снять выделение' : 'Выбрать все'}
                  </button>
               </div>
            )}

            {(words||[]).length === 0 ? <div className="text-center text-stone-400 p-8">В этой группе пока нет слов.</div> : sortedGroupWords.length === 0 ? <div className="text-center text-stone-400 p-8">По вашему запросу ничего не найдено.</div> : sortedGroupWords.map((word: Word) => (
                  <div key={word.id} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 active:scale-[0.98] transition-transform clickable-card cursor-pointer" onClick={() => onWordClick(word.id)}>
                     ...
                     <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2"><h3 className="text-lg font-bold text-stone-800 truncate">{(word.original || '').toLowerCase()}</h3> {word.partOfSpeech && <span className="text-[10px] font-bold text-stone-400 uppercase bg-stone-100 px-2 py-0.5 rounded-md shrink-0">{word.partOfSpeech}</span>}</div>
                        <p className="text-stone-500 text-sm mt-0.5 truncate">{(word.translation || '').toLowerCase()}</p><MasteryBar masteryLevel="{getEffectiveMastery(word)}"/>
                     </div>
                     <button onClick={(e) => { e.stopPropagation(); ApiClient.playTTS(word.original); }} className="p-2 mr-2 bg-stone-50 text-teal-600 rounded-full shrink-0 hover:bg-stone-100"><Volume2 className="w-5 h-5"/></button>
                     <button onClick={(e) => { e.stopPropagation(); onRemoveFromGroup(word.id); }} className="p-2 text-stone-400 hover:text-rose-500 bg-stone-50 rounded-full shrink-0"><X className="w-5 h-5"/></button>
                  </div>
            ))}
         </div>
         <AnimatePresence>
...
// НОВЫЙ КОМПОНЕНТ: МИНУТА РЕЧИ (Спикинг)
function ModeSpeaking({ onFinish, userProfile }: any) {
   const [topic, setTopic] = useState('');
   const [status, setStatus] = useState<'idle'|'recording'|'analyzing'|'done'>('idle');
   const [timeLeft, setTimeLeft] = useState(60);
   const [result, setResult] = useState<{transcript:string, feedback:string}|null>(null);
   ...
   const RANDOM_TOPICS = [
      ...
      "Talk about your dream job and why it suits you.",
   ...
   const startRecording = async () => {
      if (!topic.trim()) return alert('Сначала введите или сгенерируйте тему!');
      try {
         ...
         mediaRecorder.current.onstop = async () => {
            ...
            setStatus('analyzing');
            // Передаем level пользователя для анализа
            const res = await ApiClient.aiAnalyzeSpeech(audioBlob, topic, userProfile?.level);
            setResult(res);
            setStatus('done');
         };
   ...
   return (
      <div className="w-full flex flex-col items-center">
         <div className="w-full bg-white rounded-[2rem] p-6 text-center mb-6 border border-stone-100 shadow-sm relative overflow-hidden">
            <h2 className="text-2xl font-black text-stone-800 mb-2">Минута речи</h2>
            <p className="text-stone-500 text-sm mb-6">Тренируем беглую речь. У вас 60 секунд.</p>
            ...
            {status === 'idle' && (
               <div className="space-y-3">
                  <textarea 
                     ...
                     placeholder="О чем будете говорить? (Напр. Мой любимый фильм)"
                     className="w-full bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none focus:border-rose-500 text-sm font-medium resize-none min-h-[80px]"
                  />
                  <button onClick={handleGenerateTopic} className="text-xs font-bold text-rose-600 flex items-center justify-center gap-1 w-full active:scale-95"><Wand2 className="w-3 h-3"/> Сгенерировать случайную тему</button>
                  <button onClick={startRecording} className="w-full py-5 bg-rose-500 text-white font-bold rounded-[2rem] active:scale-95 transition-transform flex justify-center items-center gap-2 mt-4 shadow-[0_8px_20px_rgb(244,63,94,0.3)]"><Mic className="w-5 h-5"/> Начать запись</button>
               </div>
            )}

            {status === 'recording' && (
               <div className="flex flex-col items-center py-6">
                  <div className="text-5xl font-black text-rose-500 mb-4 animate-pulse">
                     00:{timeLeft.toString().padStart(2, '0')}
                  </div>
                  <button onClick={stopRecording} className="py-4 px-8 bg-stone-900 text-white font-bold rounded-2xl active:scale-95 transition-transform flex items-center gap-2"><Square className="w-5 h-5 fill-white"/> Завершить досрочно</button>
               </div>
            )}
            ...
            {status === 'analyzing' && (
               <div className="py-12 flex flex-col items-center justify-center text-stone-500">
                  ...
                  <div className="font-bold">ИИ расшифровывает и анализирует...</div>
                  <div className="text-xs mt-2">Это может занять 10-15 секунд</div>
               </div>
            )}
            ...
            {status === 'done' && result && (
               <div className="text-left space-y-4">
                  <div className="p-4 bg-stone-50 rounded-2xl border border-stone-100">
                     <div className="text-[10px] font-bold text-stone-400 uppercase mb-2">Ваша речь (расшифровка)</div>
                     <div className="text-sm font-medium text-stone-700 italic">"{result.transcript || 'Не удалось распознать речь'}"</div>
                  </div>
                  <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100">
                     <div className="text-[10px] font-bold text-rose-500/70 uppercase mb-2">Разбор от ИИ ({userProfile?.level || 'Intermediate'})</div>
                     <div className="text-sm text-stone-800 leading-relaxed whitespace-pre-wrap">{result.feedback}</div>
                  </div>
                  <button onClick={onFinish} className="w-full py-4 bg-stone-900 text-white font-bold rounded-[2rem] active:scale-95 transition-transform mt-4">Отлично</button>
               </div>
            )}
         </div>
      </div>
...
function SessionStats({ stats, onClose }: any) {
   ...
   return (
      <div className="w-full flex flex-col items-center justify-center p-4">
         <div className="bg-white rounded-[2rem] p-8 w-full max-w-xs shadow-[0_10px_40px_rgb(0,0,0,0.05)] border border-stone-100 mb-6 text-center">
            <div className="text-5xl font-black text-teal-600 mb-2">{accuracy}%</div>
            <div className="text-stone-400 font-bold mb-6 uppercase tracking-widest text-[10px]">Точность</div>
            <div className="flex justify-around text-lg font-bold">
               <div className="flex flex-col">
                  <span className="text-teal-500 text-2xl">{stats.correct}</span>
                  <span className="text-stone-400 text-[10px] uppercase mt-1">Верно</span>
               </div>
               <div className="w-px bg-stone-100 mx-4"></div>
               <div className="flex flex-col">
                  <span className="text-orange-400 text-2xl">{stats.total - stats.correct}</span>
                  <span className="text-stone-400 text-[10px] uppercase mt-1">Ошибок</span>
               </div>
            </div>
         </div>
         <button onClick={onClose} className="w-full max-w-xs py-4 bg-teal-600 text-white font-bold rounded-[1.5rem] shadow-[0_8px_20px_rgb(13,148,136,0.3)] active:scale-95">Вернуться</button>
      </div>
   );
}
...
function ModeConstructor({ words, onProgress, onFinish }: any) {
   ...
   useEffect(() => {
      ...
      ApiClient.playAutoTTS(word.original);
   }, [word, currentNum]);
   ...
   const handleHiddenInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (lastChar) {
         const char = lastChar.toLowerCase();
         const availableLetter = letters.find(l => (l.char || '').toLowerCase() === char);
         if (availableLetter || /^[a-z]$/i.test(char)) processInput(char, availableLetter);
      }
      e.target.value = '';
   ...
   if (!word) return <div className="flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-teal-600"/></div>;

   return (
      <div className="w-full flex flex-col items-center">
         <div className="w-full flex justify-between px-4 mb-4">
            <span className="text-stone-400 font-bold">{currentNum} / {queueLength}</span>
            <button onClick={() => ApiClient.playTTS(word.original)} className="text-teal-600 active:scale-90"><Volume2 className="w-5 h-5"/></button>
         </div>
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  useEffect(() => {
    ...
    const fetchBatch = async () => {
      ...
      const safeDists = Array.isArray(distsData) ? distsData : [];
      const formatted = (words||[]).map((w: any) => {
         const item = safeDists.find((d:any) => d.id === w.id);
         const dists = item && item.distractors && Array.isArray(item.distractors) && item.distractors.length >= 3 ? item.distractors.slice(0,3) : ['неверно 1', 'неверно 2', 'неверно 3'];
  ...
  useEffect(() => {
      if (phase === 'playing' && word) ApiClient.playAutoTTS(word.original);
  }, [word, currentNum, phase]);
  ...
  return (
    <div className="w-full max-w-sm flex flex-col items-center">
       ...
       <div className="w-full bg-white rounded-[2rem] p-8 text-center mb-8 border border-stone-100 shadow-sm"><h2 className="text-3xl font-black text-stone-800">{(word.original || '').toLowerCase()}</h2></div>
...
function ModeFlashcards({ words, onProgress, onFinish }: any) {
  ...
  useEffect(() => { if (isFinished) onFinish(); }, [isFinished, onFinish]);
  useEffect(() => { if (word) ApiClient.playAutoTTS(word.original); }, [word, currentNum]);
...
function ModeSentence({ words, onProgress, onFinish }: any) {
   ...
   useEffect(() => { if (isFinished) onFinish(); }, [isFinished, onFinish]);
   useEffect(() => { setInput(''); setStatus('idle'); setFb(''); if(word) ApiClient.playAutoTTS(word.original); }, [word, currentNum]);
        ...
        <div className="pb-8">
           {status === 'idle' && (
              <button data-sound="none" onClick={async () => { 
                    ...
                    const cleanWord = (word.original || '').toLowerCase();
                    if (!cleanInput.includes(cleanWord)) {
                        setStatus('incorrect');
                        setFb(`Ваше предложение должно содержать загаданное слово: "${cleanWord}"`);
                        SoundManager.play('wrong');
                        onProgress(word.id, false);
                        return;
                    }
                    ...
                    SoundManager.play(r.isCorrect ? 'correct' : 'wrong');
...
function ModeBrainstorm({ words, onProgress, onFinish }: any) {
   ...
   const startCycle = async () => {
      ...
      const formatted = activeWords.map((w:any) => {
         const item = safeDists.find((d:any) => d.id === w.id);
         const dists = item && item.distractors && Array.isArray(item.distractors) ? item.distractors.slice(0,3) : ['неверно 1', 'неверно 2', 'неверно 3'];
         return { ...w, options: [...dists, (w.translation || '').toLowerCase()].sort(() => Math.random() - 0.5) };
      });
   ...
   if (phase === 'select') return (
         <div className="w-full">
            ...
            <div className="space-y-2 mb-8 max-h-[50vh] overflow-y-auto">{(words||[]).map((w:any) => <div key={w.id} onClick={() => { const s=new Set(selected); s.has(w.id)?s.delete(w.id):s.add(w.id); setSelected(s); }} className={`p-4 rounded-2xl flex justify-between font-bold cursor-pointer clickable-card transition-colors ${selected.has(w.id) ? 'bg-purple-100 text-purple-700' : 'bg-white text-stone-700'}`}>{(w.original || '').toLowerCase()} {selected.has(w.id) && <Check className="w-5 h-5"/>}</div>)}</div>
