      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#F7F7F5]">
        <div className="bg-white/60 p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl border border-white/60 w-full max-w-sm flex flex-col items-center">
           ...
           <button onClick={handleLoginGoogle} className="w-full mb-4 py-4 bg-white border border-stone-100 text-stone-800 font-bold rounded-2xl shadow-sm flex items-center justify-center gap-3 active:scale-95 transition-transform">
             <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-6 h-6" alt="G" /> Войти через Google
           </button>
        </div>
      </div>
    );
  ...
  useEffect(() => {
    ...
    const unsubProfile = onSnapshot(doc(db, 'users', user.uid, 'profile', 'data'), snap => {
       if (snap.exists()) {
           ...
           setUserProfile(data);
           if(data.level) localStorage.setItem('userLevel', data.level); // Для доступа из функций вне React-потока
       } else {
  ...
  const handleDeleteAccount = async () => {
    if(window.confirm("Вы уверены? Это навсегда удалит ваш аккаунт и все слова.")) {
      try {
         ...
         alert("В целях безопасности необходимо перезайти в аккаунт перед удалением.");
         signOut(auth);
      }
    }
  };
                 ...
                 <motion.div key="tab-settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8">
                     <div className="bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 mb-6">
                        ...
                        <div className="border-t border-stone-100 pt-6">
                           ...
                           <p className="text-stone-500 text-xs mb-4">От уровня зависит сложность примеров и словаря.</p>
...
function ModeBrainstorm({ words, onProgress, onFinish }: any) {
   ...
   if (phase === 1) return (
         <div className="w-full"><h2 className="text-2xl font-black text-stone-800 mb-2">Брейншторм</h2><p className="text-stone-500 mb-6">Выберите слова, которые хотите прогнать через цикл тренировки.</p><div className="space-y-2 mb-8 max-h-[50vh] overflow-y-auto">{pool.map(w => <div key={w.id} onClick={() => { const s=new Set(selected); s.has(w.id)?s.delete(w.id):s.add(w.id); setSelected(s); }} className={`p-4 rounded-2xl flex justify-between font-bold cursor-pointer transition-colors ${selected.has(w.id) ? 'bg-purple-100 text-purple-700' : 'bg-white text-stone-700'}`}>{w.original} {selected.has(w.id) && <Check className="w-5 h-5"/>}</div>)}</div><button onClick={() => { if(selected.size>0) setPhase(2); else alert('Выберите слова'); }} className="w-full bg-purple-600 text-white font-bold py-5 rounded-[2rem]">Начать цикл ({selected.size})</button></div>
   );
...
function OnboardingModal({ user, onSave }: any) {
  ...
  const slides = [
    { icon: <Wand2 className="w-8 h-8 text-teal-600" />, title: "Магия ИИ", text: "Добавляйте слова, а нейросеть сама найдет идеальный перевод, транскрипцию (US/UK) и подробное объяснение из Cambridge Dictionary." },
    { icon: <Edit3 className="w-8 h-8 text-sky-600" />, title: "Полный контроль", text: "Словарь — это ваш конспект. В любой момент вы можете отредактировать перевод, сгенерировать новый пример или разложить слова по группам." },
    { icon: <Brain className="w-8 h-8 text-purple-600" />, title: "5 режимов тренировки", text: "От классических карточек и викторины до сборки слов из букв и общения с ИИ для проверки ваших предложений на грамматику." }
  ];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} transition={{type:'spring', stiffness: 200, damping: 25}} className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-[0_20px_60px_rgb(0,0,0,0.1)] relative z-10">
        ...
        {step < 3 ? (
          <div className="p-8 text-center">
            ...
            <div className="flex justify-center gap-2 mt-8">
               {[0,1,2].map(i => <div key={i} className={`w-2 h-2 rounded-full transition-colors ${step === i ? 'bg-teal-500' : 'bg-stone-200'}`} />)}
            </div>
          </div>
        ) : (
          <div className="p-8">
        ...
        <div className="p-6 bg-stone-50 border-t border-stone-100 flex gap-3">
          ...
          <button onClick={() => { if (step < 3) setStep(s=>s+1); else onSave(level); }} className="flex-1 bg-stone-900 text-white font-bold py-4 rounded-2xl active:scale-95 transition-transform">
            {step < 3 ? 'Далее' : 'Начать обучение'}
          </button>
        </div>
      </motion.div>
    </motion.div>
