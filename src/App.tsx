...
import {
  ...
  BookOpen, Trash2, FolderPlus, ArrowLeft, Edit3
} from 'lucide-react';
...
function MainApp({ user }: { user: User }) {
  ...
  // Изолированный стейт для тренировки, чтобы не перезагружался UI при сохранении прогресса
              ...
              <AnimatePresence mode="wait">
                 ...
                 {activeTab === 'settings' && (
                    <motion.div key="tab-settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8">
                        ...
                        <div className="space-y-3">
                          <button onClick={() => signOut(auth)} className="w-full py-4 bg-stone-200/50 text-stone-700 font-bold rounded-2xl active:scale-95 transition-transform flex justify-center items-center gap-2"><X className="w-5 h-5"/> Выйти</button>
                          <button onClick={handleDeleteAccount} className="w-full py-4 bg-orange-50 text-orange-500 font-bold rounded-2xl active:scale-95 transition-transform flex items-center justify-center gap-2"><Trash2 className="w-5 h-5"/> Удалить аккаунт</button>
                        </div>
                    </motion.div>
                 )}
              </AnimatePresence>
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           ...
           <div className="flex-1 overflow-y-auto space-y-4 pb-6 hide-scrollbar">
              ...
              <div className="space-y-1">
                 <div className="flex justify-between items-center"><label className="text-[10px] font-bold text-stone-400 uppercase">Пример ИИ</label><button onClick={handleRegenerateExample} disabled={isRegenerating} className="text-[10px] font-bold text-teal-600 flex items-center gap-1 active:scale-95">Перегенерировать</button></div>
