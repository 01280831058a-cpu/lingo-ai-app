...
function AuthScreen() {
   ...
   {/* Вставляем картинку жабы. Класс object-contain не даст ей сплющиться */}
...
function MainApp({ user }: { user: User }) {
  ...
  const handleUpdateProgress = (wordId: string, isCorrect: boolean, mode: string = 'general') => {
    ...
    // Новая суровая логика прогресса
  ...
  <div className="flex items-center gap-3 mb-10 px-2 mt-4">
     {/* Здесь картинка поменьше (w-10 h-10) */}
                 ...
                 <motion.div key="tab-dict" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="p-4 md:p-8 space-y-3 pb-32 md:pb-8">
                   ...
                   {words.length > 0 && (
                      <div className="flex justify-between items-center px-1 mb-2">
                         {/* ДОБАВЛЕНО ОТОБРАЖЕНИЕ ОБЩЕГО КОЛИЧЕСТВА СЛОВ */}
                   ...
                   {words.length === 0 ? <div className="text-center text-stone-400 py-12">Словарь пуст.</div> : sortedDictWords.length === 0 ? <div className="text-center text-stone-400 py-12">По вашему запросу ничего не найдено.</div> : sortedDictWords.map(word => (
                     <div key={word.id} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 active:scale-[0.98] transition-transform clickable-card cursor-pointer" onClick={() => setViewingWordId(word.id)}>
                        ...
                        {/* ДОБАВЛЕН класс min-w-0 для предотвращения распирания */}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                             {/* ДОБАВЛЕН truncate для длинных слов */}
                          ...
                          {/* ДОБАВЛЕН truncate (вместо line-clamp-1) для гарантии обрезки длинных переводов */}
                      ...
                      <div className="space-y-4">
                         <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                            <div className="flex items-center justify-between mb-4">
                               ...
                               <button onClick={() => ApiClient.playTTS(activeTab.wordData?.original || activeTab.original)} className="w-10 h-10 flex items-center justify-center bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200 active:scale-90 transition-all">
                                     ...
                                     {(activeTab.wordData?.relatedWords || []).map((rw:any, i:number) => {
                                        ...
                                        return (
                                           <div key={i} className="flex items-center justify-between bg-white p-3 rounded-xl border border-stone-200 shadow-sm">
                                              ...
                                              <button onClick={()=>{}} disabled={alreadyInTabs} className="w-8 h-8 flex shrink-0 items-center justify-center bg-teal-50 text-teal-600 rounded-full active:scale-90 disabled:opacity-50 disabled:bg-stone-100 disabled:text-stone-300">{alreadyInTabs ? <Check className="w-4 h-4"/> : <Plus className="w-5 h-5"/>}</button>
                                           </div>
                                        );
                                     })}
                                  </div>
                ...
                <div className="space-y-4">
                   <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                      <div className="flex items-center justify-between mb-4">
                         ...
                         <button onClick={() => ApiClient.playTTS(activeTab.wordData?.original || activeTab.original)} className="w-10 h-10 flex items-center justify-center bg-teal-100 text-teal-700 rounded-full hover:bg-teal-200 active:scale-90 transition-all">
                            <Volume2 className="w-5 h-5"/>
                         </button>
                      </div>
                      <div className="space-y-5">
           ...
           <div className="flex-1 overflow-y-auto space-y-4 pb-6 hide-scrollbar">
              <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                 <div className="flex items-center justify-between mb-4">
                    <div className="w-10"></div>
                    <div className="flex items-center gap-2">
                       <h3 className="text-3xl font-black text-stone-800">{(word.original || '').toLowerCase()}</h3>
                       {word.partOfSpeech && <span className="bg-stone-200 text-stone-600 font-bold px-2 py-1 rounded-lg text-xs uppercase">{word.partOfSpeech}</span>}
                    </div>
                    <button onClick={() => ApiClient.playTTS(word.original)} className="w-10 h-10 flex items-center justify-center bg-teal-100 text-teal-700 rounded-full hover:bg-teal-200 active:scale-90 transition-all">
                       <Volume2 className="w-5 h-5"/>
                    </button>
                 </div>
