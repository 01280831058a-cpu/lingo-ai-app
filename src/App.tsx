...
// Вычисление динамического уровня знания с учетом времени (Деградация памяти)
export const getEffectiveMastery = (word: Partial<Word>) => {
    ...
    // Спустя 1 неделю: -10, за каждую следующую еще -5
   ...
   return (
       <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
           ...
           <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2rem] p-6 shadow-2xl relative z-10 w-full max-w-sm text-center">
              {type === 'error' ? <XOctagon className="w-12 h-12 text-rose-500 mx-auto mb-4"/> : type === 'success' ? <CheckCircle2 className="w-12 h-12 text-teal-500 mx-auto mb-4"/> : <Info className="w-12 h-12 text-sky-500 mx-auto mb-4"/>}
...
function AuthScreen() {
   ...
   const handleGoogle = async () => { try { await signInWithPopup(auth, new GoogleAuthProvider()); } catch(e:any){ setAlertData({title: "Ошибка", message: translateAuthError(e.code), type: 'error'}); } };
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      } catch(e:any) { setAlertData({title:"Ошибка", message: translateAuthError(e.code), type: 'error'}); }
      setLoading(false);
   };
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-y-auto ${!activeTrainingMode ? 'md:ml-64 pb-24 md:pb-0' : ''}`}>
        <div className="max-w-4xl mx-auto w-full relative min-h-full flex flex-col">
          ...
          <AnimatePresence>
            ...
            {viewingWordId && <WordEditorModal words={words} word={words.find(w=>w.id===viewingWordId)!} groups={groups} userProfile={userProfile} user={user} onClose={() => setViewingWordId(null)} onReset={() => setAppConfirm({title:"Сброс прогресса", message:"Сбросить прогресс этого слова?", onConfirm: ()=>resetProgress([viewingWordId])})} onWordClick={(id:string)=>setViewingWordId(id)} onSave={(w:any) => { updateDoc(doc(db,'users',user.uid,'words',w.id),w); setViewingWordId(null); }} onDelete={() => { setAppConfirm({title:"Удаление", message:"Удалить это слово из словаря?", onConfirm:()=>{deleteWords([viewingWordId!]); setViewingWordId(null);}}) }} />}
...
function AddWordModal({ words, userProfile, groups, onClose, onSaveWord, onWordClick, user }: any) {
  ...
  const handleAnalyze = async () => {
    ...
    const result = await ApiClient.aiGenerateWord(activeTab.original, userProfile?.level);
    updateTab(activeTabId, { status: 'done', wordData: result, selectedTranslation: result.translationOptions?.[0] || result.translation || '' });
  };

  const handleAddRelated = async (rw: any) => {
     const newId = Date.now().toString();
     setTabs(prev => [...prev, { id: newId, original: rw.word, status: 'analyzing', wordData: {}, selectedTranslation: '', groupIds: new Set() }]);
     setActiveTabId(newId);
     const result = await ApiClient.aiGenerateWord(rw.word, userProfile?.level);
     setTabs(prev => prev.map(t => t.id === newId ? { ...t, status: 'done', wordData: result, selectedTranslation: result.translationOptions?.[0] || result.translation || rw.translation } : t));
  };

  const handleSaveTab = () => {
     const wToSave = { original: activeTab.wordData.original || activeTab.original, ...activeTab.wordData, translation: activeTab.selectedTranslation, groupIds: Array.from(activeTab.groupIds) };
     onSaveWord(wToSave);
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
        ...
        <div className="flex overflow-x-auto gap-2 pb-2 mb-4 hide-scrollbar border-b border-stone-100">
           {tabs.map((t) => (
               <div key={t.id} onClick={() => setActiveTabId(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-bold shrink-0 cursor-pointer transition-all border-b-2 ${activeTabId === t.id ? 'bg-teal-50 text-teal-700 border-teal-500' : 'bg-transparent text-stone-400 border-transparent hover:bg-stone-50'}`}>
                  {t.wordData?.original || t.original || 'Новое слово'}
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          {activeTab.status === 'idle' && (
             <>
                <input autoFocus placeholder="Введите слово..." value={activeTab.original} onChange={e => updateTab(activeTabId, { original: e.target.value })} className="w-full bg-stone-50 px-6 py-5 rounded-[2rem] text-lg font-bold border border-stone-200 outline-none focus:border-teal-500" />
            ...
            <div className="space-y-4">
               <div className="bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
                  <div className="flex items-center justify-center gap-2 mb-6">
                     <h3 className="text-3xl font-black text-stone-800">{activeTab.wordData.original || activeTab.original}</h3>
                      ...
                      <div>
                         ...
                         <div className="flex flex-wrap justify-center gap-2 mb-2">
                            {activeTab.wordData.translationOptions?.map((opt:string, i:number) => (
                               <button key={i} onClick={()=>updateTab(activeTabId, { selectedTranslation: opt })} className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${activeTab.selectedTranslation === opt ? 'bg-teal-500 text-white border-teal-500 shadow-sm' : 'bg-white text-stone-600 border-stone-200'}`}>{opt}</button>
                            ))}
                         </div>
                      </div>
                     ...
                     <div className="mt-6 pt-4 border-t border-stone-200/50 text-center">
                        ...
                        <div className="space-y-2">
                           {relatedList.map((rw:any, i:number) => {
                              const alreadyInTabs = tabs.some(t => (t.wordData?.original || t.original).toLowerCase() === rw.word.toLowerCase());
                              return (
...
function WordEditorModal({ word, words, groups, userProfile, user, onClose, onSave, onDelete, onReset, onWordClick }: any) {
  ...
  const handleSave = () => {
     onSave({ ...word, original, translation, cambridgeTranslation, transcriptionUK, transcriptionUS, groupIds: Array.from(groupIds), examples: exampleText ? [{ text: exampleText, translation: exampleTranslation }] : [] });
     onClose();
  };

  const handleAddRelated = async (rw: any) => {
     setLoadingRelated(rw.word);
     const result = await ApiClient.aiGenerateWord(rw.word, userProfile?.level);
     const id = doc(collection(db, 'users', user.uid, 'words')).id;
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: rw.word, translation: result.translationOptions?.[0] || result.translation || rw.translation, createdAt: Date.now(), masteryLevel: 0, lastPracticed: Date.now() });
     ...
     <div className="fixed inset-0 z-[100] flex flex-col justify-end items-center">
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           ...
           {relatedTabs.length > 0 && (
              <div className="flex overflow-x-auto gap-2 pb-2 mb-4 hide-scrollbar border-b border-stone-100">
                  <button className="px-4 py-2 bg-teal-600 text-white rounded-xl text-sm font-bold shrink-0 shadow-sm">{word.original}</button>
                  {relatedTabs.map((rw: any, i: number) => {
                      const existing = words.find((w:any) => w.original.toLowerCase() === rw.word.toLowerCase());
                      if (existing) {
                           return <button key={i} onClick={() => onWordClick(existing.id)} className="px-4 py-2 bg-stone-100 text-stone-600 rounded-xl text-sm font-bold shrink-0 active:scale-95 transition-transform">{rw.word}</button>
                      } else {
                           return <button key={i} onClick={() => handleAddRelated(rw)} disabled={loadingRelated === rw.word} className="px-4 py-2 bg-white border border-stone-200 border-dashed text-stone-400 rounded-xl text-sm font-bold shrink-0 flex items-center gap-1 active:scale-95 transition-transform disabled:opacity-50">{loadingRelated === rw.word ? <Loader2 className="w-3 h-3 animate-spin"/> : <Plus className="w-3 h-3"/>} {rw.word}</button>
                      }
                  })}
              </div>
           )}
            ...
            {words.length === 0 ? <div className="text-center text-stone-400 p-8">В этой группе пока нет слов.</div> : words.map((word: Word) => (
                  <div key={word.id} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 active:scale-[0.98] transition-transform">
                     ...
                     <div className="flex-1 cursor-pointer" onClick={() => onWordClick(word.id)}>
                        <div className="flex items-center gap-2"><h3 className="text-lg font-bold text-stone-800">{word.original}</h3> {word.partOfSpeech && <span className="text-[10px] font-bold text-stone-400 uppercase bg-stone-100 px-2 py-0.5 rounded-md">{word.partOfSpeech}</span>}</div>
                        <p className="text-stone-500 text-sm mt-0.5 line-clamp-1">{word.translation}</p><MasteryBar masteryLevel={getEffectiveMastery(word)} />
                     </div>
...
function SessionStats({ stats, onClose }: any) {
   ...
   return <div className="w-full text-center flex flex-col items-center"><div className="bg-white rounded-[2rem] p-10 w-full shadow-[0_10px_40px_rgb(0,0,0,0.05)] border border-stone-100 mb-8"><div className="text-6xl font-black text-teal-600 mb-2">{accuracy}%</div><div className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs">Точность</div><div className="flex justify-around text-lg font-bold"><div className="flex flex-col"><span className="text-teal-500 text-2xl">{stats.correct}</span><span className="text-stone-400 text-xs uppercase">Верно</span></div><div className="flex flex-col"><span className="text-orange-400 text-2xl">{stats.total - stats.correct}</span><span className="text-stone-400 text-xs uppercase">Ошибок</span></div></div></div><button onClick={onClose} className="w-full py-4 bg-teal-600 text-white font-bold rounded-[2rem] shadow-[0_8px_20px_rgb(13,148,136,0.3)] active:scale-95">Вернуться</button></div>;
}
...
function useTrainingQueue(initialWords: Word[]) {
   ...
   }, [initialWords.length]); // Initialize ONLY when length changes (so it doesn't reset randomly)
...
function ModeQuiz({ words, onProgress, onFinish }: any) {
  ...
  const [phase, setPhase] = useState<'loading' | 'playing'>(isPreloaded ? 'playing' : 'loading');
  ...
  useEffect(() => {
    ...
    const fetchBatch = async () => {
      ...
      setQuizData(formatted); setPhase('playing');
    };
    fetchBatch();
  }, [words, isPreloaded]);
...
function OnboardingModal({ user, onSave }: any) {
  ...
  const slides = [
    { icon: <Wand2 className="w-10 h-10 text-teal-600" />, title: "Академический ИИ-словарь", text: "Больше никаких гугл-переводчиков. Добавьте слово, и наша нейросеть выдаст идеальный перевод, британскую и американскую транскрипции, а также развернутое объяснение в стиле Cambridge Dictionary." },
    { icon: <Edit3 className="w-10 h-10 text-sky-600" />, title: "Полный контроль", text: "Ваш словарь — это ваш личный конспект. Вы можете группировать слова, сбрасывать их прогресс или переписывать значения. А если пример от ИИ показался скучным — просто перегенерируйте его в один клик!" },
    { icon: <Brain className="w-10 h-10 text-purple-600" />, title: "5 режимов тренировки", text: "Мы создали систему, которая не даст вам забыть. Классические карточки, умная викторина, конструктор слов (где за ошибки вас штрафуют), Брейншторм для самых трудных слов и ИИ-проверка предложений." },
    { icon: <Calendar className="w-10 h-10 text-orange-400" />, title: "Ваш прогресс", text: "Секрет изучения языков — в регулярности. Установите ежедневную цель, отслеживайте свой стрик (дни подряд) и наблюдайте, как календарь заполняется вашими победами." }
  ];
      ...
      <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} transition={{type:'spring', stiffness: 200, damping: 25}} className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-[0_20px_60px_rgb(0,0,0,0.1)] relative z-10">
        ...
        {step < 4 ? (
          <div className="p-8 text-center">
            <div className="w-20 h-20 mx-auto bg-stone-50 rounded-3xl flex items-center justify-center mb-6 shadow-sm">{slides[step].icon}</div>
          ...
          <div className="p-8">
             ...
             <h2 className="text-2xl font-black mb-2 text-stone-800 text-center">Ваш уровень?</h2><p className="text-stone-500 text-sm mb-6 text-center">От уровня зависит лексика и сложность примеров ИИ.</p>
             <div className="grid grid-cols-2 gap-3 mb-6">{LEVELS.map(l => <button key={l} onClick={() => setLevel(l)} className={`p-4 rounded-2xl border-2 font-bold text-center transition-colors text-sm ${level === l ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-stone-100 text-stone-500'}`}>{l}</button>)}</div>
             <h2 className="text-xl font-black mb-2 text-stone-800 text-center">Цель в день</h2>
             <div className="flex gap-2 mb-6">{[5,10,15,30].map(m => <button key={m} onClick={() => setGoal(m)} className={`flex-1 p-3 rounded-2xl border-2 font-bold text-center transition-colors text-sm ${goal === m ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-stone-100 text-stone-500'}`}>{m}</button>)}</div>
             {deferredPrompt && <button onClick={()=>{ deferredPrompt.prompt(); deferredPrompt.userChoice.then(()=>{ setDeferredPrompt(null); }) }} className="w-full py-4 bg-teal-50 text-teal-600 font-bold rounded-2xl flex items-center justify-center gap-2 border border-teal-100 mt-2 active:scale-95"><Download className="w-5 h-5"/> Установить на экран "Домой"</button>}
          </div>
        )}
        <div className="p-6 bg-stone-50 border-t border-stone-100 flex gap-3">
          ...
          <button onClick={() => { if (step < 4) setStep(s=>s+1); else onSave(level, goal); }} className="flex-1 bg-stone-900 text-white font-bold py-4 rounded-2xl active:scale-95 transition-transform">{step < 4 ? 'Понятно' : 'Начать обучение'}</button>
        </div>
      </motion.div>
    </motion.div>
  );
