...
function WordEditorModal({ word, words, groups, userProfile, user, onClose, onSave, onDelete, onReset, onWordClick }: any) {
  ...
  const handleAddRelated = async (rw: any) => {
     ...
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: rw.word, translation: result.translationOptions?.[0] || result.translation || rw.translation, createdAt: Date.now(), masteryLevel: 0 });
        ...
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] p-6 shadow-2xl flex flex-col max-h-[90vh] relative z-10">
           ...
           <div className="flex justify-between items-center mb-6 shrink-0">
              ...
              <div className="flex items-center gap-2">
                 <button onClick={onReset} className="p-2 bg-orange-50 text-orange-500 rounded-full active:scale-90 transition-transform"><RotateCcw className="w-5 h-5" /></button>
            ...
            {words.length === 0 ? <div className="text-center text-stone-400 p-8">В этой группе пока нет слов.</div> : words.map((word: Word) => (
                  <div key={word.id} className="bg-white p-5 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-white/60 flex items-center gap-4 active:scale-[0.98] transition-transform">
                     ...
                     <div className="flex-1 cursor-pointer" onClick={() => onWordClick(word.id)}>
                        <div className="flex items-center gap-2"><h3 className="text-lg font-bold text-stone-800">{word.original}</h3> {word.partOfSpeech && <span className="text-[10px] font-bold text-stone-400 uppercase bg-stone-100 px-2 py-0.5 rounded-md">{word.partOfSpeech}</span>}</div>
                        <p className="text-stone-500 text-sm mt-0.5 line-clamp-1">{word.translation}</p><MasteryBar masteryLevel={word.masteryLevel || 0} />
                     </div>
   ...
   if (phase === 1) return (
         <div className="w-full">
            ...
            <div className="flex justify-between items-center mb-6">
               ...
               <button onClick={() => selected.size === words.length ? setSelected(new Set()) : setSelected(new Set(words.map((w:any)=>w.id)))} className="text-sm font-bold text-teal-600 active:opacity-70 px-2 py-1 bg-teal-50 rounded-lg">{selected.size === words.length ? 'Снять все' : 'Выбрать все'}</button>
            </div>
            ...
            <button onClick={() => { if(selected.size>0) setPhase(2); else alert('Выберите слова'); }} className="w-full bg-purple-600 text-white font-bold py-5 rounded-[2rem] active:scale-95 transition-transform">Начать цикл ({selected.size})</button>
         </div>
   );
   
   if (phase === 2) return <div className="w-full h-full flex flex-col"><h3 className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs text-center">Этап 1: Карточки</h3><ModeFlashcards words={activeWords} onProgress={onProgress} onFinish={() => setPhase(3)} /></div>;
   if (phase === 3) return <div className="w-full h-full flex flex-col"><h3 className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs text-center">Этап 2: Викторина</h3><ModeQuiz words={activeWords} onProgress={onProgress} onFinish={() => setPhase(4)} /></div>;
   if (phase === 4) return <div className="w-full h-full flex flex-col"><h3 className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs text-center">Этап 3: Правописание</h3><ModeConstructor words={activeWords} onProgress={onProgress} onFinish={() => setPhase(5)} /></div>;
   if (phase === 5) return <div className="w-full h-full flex flex-col"><h3 className="text-stone-400 font-bold mb-8 uppercase tracking-widest text-xs text-center">Этап 4: Практика фраз</h3><ModeSentence words={activeWords} onProgress={onProgress} onFinish={onFinish} /></div>;
