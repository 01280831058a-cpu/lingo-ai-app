...
import {
  ...
  LogOut, Wand2, GraduationCap, Download, Mail, Calendar, RotateCcw
} from 'lucide-react';
...
function AddWordModal({ userProfile, groups, onClose, onSave, user }: any) {
  ...
  const addAlternative = async (alt: string) => {
     ...
     await setDoc(doc(db, 'users', user.uid, 'words', id), { ...result, id, original: alt, translation: result.translationOptions?.[0] || result.translation, createdAt: Date.now(), masteryLevel: 0 });
     setStatus('done');
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-6 hide-scrollbar">
          ...
          {status === 'done' && (
            <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                           {wordData.relatedWords.map((rw:any, i:number) => (
                              <div key={i} className="flex items-center justify-between bg-white p-3 rounded-xl border border-stone-200 shadow-sm">
                                 <div className="text-left"><span className="font-bold text-stone-800">{rw.word}</span> <span className="text-[10px] text-stone-400 bg-stone-100 px-1.5 py-0.5 rounded ml-1 uppercase">{rw.partOfSpeech}</span><div className="text-xs text-stone-500 mt-1">{rw.translation}</div></div>
                                 <button onClick={()=>addAlternative(rw.word)} disabled={loadingRelated === rw.word} className="w-8 h-8 flex shrink-0 items-center justify-center bg-teal-50 text-teal-600 rounded-full active:scale-90 disabled:opacity-50">{loadingRelated === rw.word ? <Loader2 className="w-4 h-4 animate-spin"/> : <Plus className="w-5 h-5"/>}</button>
                              </div>
                           ))}
                        </div>
                     </div>
               ...
               <button onClick={() => onSave({ original: wordData.original || original, ...wordData, translation: selectedTranslation, groupIds: Array.from(selectedGroups) })} className="w-full py-5 mt-4 bg-stone-900 text-white font-bold rounded-[2rem] active:scale-95 transition-transform">Сохранить в словарь</button>
            </div>
          )}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} transition={{type:'spring', stiffness: 200, damping: 25}} className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-[0_20px_60px_rgb(0,0,0,0.1)] relative z-10">
        ...
        {step < 4 ? (
          ...
          <div className="p-8">
             ...
             <p className="text-stone-500 text-sm mb-6 text-center">Это критически важно: от уровня зависит лексика и сложность примеров ИИ.</p>
