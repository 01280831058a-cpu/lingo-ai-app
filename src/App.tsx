                   ...
                   <>
                      <div className="grid grid-cols-2 gap-3">
                         <div className="col-span-1 space-y-1">
                             <label className="text-[10px] font-bold text-stone-400 uppercase">Сложность</label>
                             <select value={genLevel} onChange={e=>setGenLevel(e.target.value)} className="w-full bg-stone-50 px-3 py-3 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold text-stone-700">
                                 {LEVELS.map(l=><option key={l} value={l}>{l}</option>)}
                             </select>
                         </div>
                         <div className="col-span-1 space-y-1"><label className="text-[10px] font-bold text-stone-400 uppercase">Кол-во (до 50)</label><input type="number" min="1" max="50" value={count} onChange={e => setCount(e.target.value)} className="w-full bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold" /></div>
                      </div>
                      <div className="space-y-1"><label className="text-[10px] font-bold text-stone-400 uppercase">Тема (или оставьте пустым для файла)</label><input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Напр. Путешествия" className="w-full bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold" /></div>
