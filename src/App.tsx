...
function GenerateWordsModal({ words, userProfile, groups, onClose, onSaveWord, onWordClick }: any) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col justify-end items-center">
      <div className="absolute inset-0 bg-stone-900/40 backdrop-blur-sm" onClick={onClose} />
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="bg-white w-full max-w-lg rounded-t-[2rem] shadow-2xl flex flex-col max-h-[90vh] relative z-10 overflow-hidden">
        
        <div className="p-6 pb-2 border-b border-stone-100 flex overflow-x-auto gap-2 hide-scrollbar">
           {tabs.map((t) => (
               <div key={t.id} onClick={() => setActiveTabId(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-bold shrink-0 cursor-pointer transition-all border-b-2 ${activeTabId === t.id ? 'bg-purple-50 text-purple-700 border-purple-500' : 'bg-transparent text-stone-400 border-transparent hover:bg-stone-50'}`}>
                  {t.id === 'generator' ? <Sparkles className="w-4 h-4"/> : (t.wordData?.original || t.original || '...').toLowerCase()}
                  {t.id !== 'generator' && <button onClick={(e) => handleCloseTab(t.id, e)} className="p-0.5 rounded-full hover:bg-stone-200"><X className="w-3 h-3"/></button>}
               </div>
           ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 hide-scrollbar relative">
                ...
                {(activeTab.generatedWords || []).length === 0 ? (
                   <>
                      <div className="space-y-2">
                          <label className="text-[10px] font-bold text-stone-400 uppercase">Сложность и количество</label>
                          <div className="flex gap-2">
                             <select value={genLevel} onChange={e=>setGenLevel(e.target.value)} className="flex-1 bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none font-bold text-stone-700">{LEVELS.map(l=><option key={l}>{l}</option>)}</select>
                             <input type="number" min="1" max="30" value={count} onChange={e => setCount(e.target.value)} className="w-20 bg-stone-50 px-4 py-3 rounded-2xl border border-stone-200 outline-none font-bold" />
                          </div>
                      </div>
                      <div className="space-y-1">
                          <label className="text-[10px] font-bold text-stone-400 uppercase">О чем должны быть слова?</label>
                          <textarea value={topic} onChange={e => { setTopic(e.target.value); e.target.style.height = 'auto'; e.target.style.height = e.target.scrollHeight + 'px'; }} placeholder="Напр. Собеседование в IT компанию..." className="w-full bg-stone-50 px-5 py-4 rounded-2xl border border-stone-200 outline-none focus:border-purple-500 font-bold resize-none overflow-hidden min-h-[80px]" />
                      </div>
                      <label className="w-full flex items-center justify-center gap-2 bg-purple-50 text-purple-700 border border-purple-200 border-dashed py-5 rounded-2xl cursor-pointer font-bold text-sm">
                         <UploadCloud className="w-5 h-5"/> {fileName || 'Прикрепить PDF (до 5 стр.)'}
                         <input type="file" accept=".pdf,.txt" className="hidden" onChange={handleFileUpload} />
                      </label>
                      <button onClick={handleGenerate} disabled={loading} className="w-full py-5 bg-purple-600 text-white font-bold rounded-[1.5rem] active:scale-95 disabled:opacity-50 flex justify-center items-center gap-2 shadow-lg shadow-purple-200">
                         {loading ? <Loader2 className="w-5 h-5 animate-spin"/> : <Sparkles className="w-5 h-5"/>} Сгенерировать
                      </button>
                   </>
                ) : (
                   <div className="pb-20">
                      <div className="flex justify-between mb-4"><button onClick={selectAll} className="text-xs font-bold text-purple-600 flex items-center gap-1"><CheckCircle2 className="w-4 h-4"/> {selectedGenWords.size === (activeTab.generatedWords || []).length ? 'Снять все' : 'Выбрать все'}</button><button onClick={handleReset} className="text-xs font-bold text-stone-400">Сбросить</button></div>
                      <div className="space-y-2">
                         {(activeTab.generatedWords || []).map((w:any, i:number) => (
                               <div key={i} onClick={() => toggleSelect(w.word)} className={`flex items-center p-4 rounded-2xl border cursor-pointer transition-all ${selectedGenWords.has(w.word) ? 'border-purple-500 bg-purple-50' : 'border-stone-100 bg-white'}`}>
                                  <div className={`w-5 h-5 rounded-full border-2 mr-3 flex items-center justify-center ${selectedGenWords.has(w.word) ? 'bg-purple-500 border-purple-500' : 'border-stone-200'}`}>{selectedGenWords.has(w.word) && <Check className="w-3 h-3 text-white"/>}</div>
                                  <div className="flex-1"><span className="font-bold text-stone-800">{(w.word || '').toLowerCase()}</span><div className="text-xs text-stone-400">{(w.translation || '').toLowerCase()}</div></div>
                                  <button onClick={(e)=>{ e.stopPropagation(); handleAnalyzeWord(w); }} className="p-2 bg-stone-50 rounded-full"><Plus className="w-4 h-4"/></button>
                               </div>
                         ))}
                      </div>
                   </div>
          ...
          {activeTab.type === 'word' && (
             <div className="pb-24">
                {activeTab.status === 'analyzing' ? <div className="py-20 text-center text-stone-400"><Loader2 className="w-8 h-8 animate-spin mx-auto mb-2"/>Загрузка данных...</div> : (
                  <div className="space-y-5">
                     <div className="bg-stone-50 p-6 rounded-3xl text-center">
                        ...
                        <div className="text-teal-600 font-bold mt-2">{(activeTab.selectedTranslation || '').toLowerCase()}</div>
                     </div>
                     <div className="grid grid-cols-2 gap-2">
                        {(activeTab.wordData?.translationOptions || []).map((opt:any)=><button key={opt} onClick={()=>updateWordTab(activeTabId, {selectedTranslation: opt})} className={`p-3 rounded-xl border font-bold text-sm ${activeTab.selectedTranslation === opt ? 'bg-teal-500 text-white' : 'bg-white'}`}>{opt}</button>)}
                     </div>
                     <div className="p-4 bg-teal-50 rounded-2xl text-sm italic">"{activeTab.wordData?.examples?.[0]?.text}"</div>
                     {(groups||[]).length > 0 && (
                        <div className="flex flex-wrap gap-2">
                           {(groups||[]).map((g:any) => <button key={g.id} onClick={()=>{const s=new Set(activeTab.groupIds); s.has(g.id)?s.delete(g.id):s.add(g.id); updateWordTab(activeTabId, {groupIds:s});}} className={`px-3 py-2 rounded-xl text-xs font-bold border ${activeTab.groupIds?.has(g.id)?'bg-stone-800 text-white':'bg-white'}`}>{g.name}</button>)}
                        </div>
                     )}
                  </div>
                )}
        ...
        <AnimatePresence>
           {activeTab.type === 'word' && activeTab.status === 'done' && (
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-md border-t border-stone-100">
                 <button onClick={handleSaveTab} className="w-full py-4 bg-stone-900 text-white font-bold rounded-2xl shadow-xl active:scale-95">Сохранить и далее</button>
              </motion.div>
           )}
           {activeTab.type === 'list' && selectedGenWords.size > 0 && (
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-md border-t border-stone-100">
                 <button onClick={handleAddSelectedToTabs} className="w-full py-4 bg-purple-600 text-white font-bold rounded-2xl shadow-xl active:scale-95">Добавить во вкладки ({selectedGenWords.size})</button>
              </motion.div>
           )}
        </AnimatePresence>
      </motion.div>
