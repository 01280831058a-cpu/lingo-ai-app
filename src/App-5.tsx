...
function MainApp({ user }: { user: User }) {
  ...
  <div className="flex flex-wrap items-center gap-2">
    <h3 className="text-lg font-bold text-stone-800 break-words max-w-full">{(word.original || '').toLowerCase()}</h3> 
  ...
  <p className="text-stone-500 text-sm mt-0.5 line-clamp-2 break-words">{(word.translation || '').toLowerCase()}</p>
                         ...
                         {(activeTab.generatedWords || []).map((w:any, i:number) => {
                            ...
                            return (
                               <div key={i} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-stone-200 shadow-sm gap-3">
                                  ...
                                  <div className="flex-1 cursor-pointer clickable-card min-w-0" onClick={() => toggleSelect(w.word)}>
   <div className="font-bold text-stone-800 text-lg break-words">{cleanWWord}</div>
   <div className="text-sm text-stone-500 break-words line-clamp-2">{(w.translation || '').toLowerCase()}</div>
</div>
                                  {existingInDict ? (
         ...
         <div className="flex items-center px-4 md:px-8 pb-4 border-b border-stone-200/50 sticky top-0 z-10 bg-[#F7F7F5]/80 backdrop-blur-xl">
            ...
            <div className="flex-1 ml-2">
               {isEditingTitle ? (
                  ...
                  <h2 className="text-xl font-bold text-stone-800 flex items-center gap-2 cursor-pointer" onClick={()=>setIsEditingTitle(true)}>{group.name} <Edit3 className="w-4 h-4 text-stone-400"/></h2>
               )}
